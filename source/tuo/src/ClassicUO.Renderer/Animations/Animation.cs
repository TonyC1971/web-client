...
namespace ClassicUO.Renderer.Animations
{
    public sealed class Animations
    {
        ...
        // Last AnimationsLoader.FilesGeneration we reconciled against. When the
        // loader opens a background-deferred giant anim file post-boot (Ultima
        // Memento streams anim2-5.mul after world-entry), it bumps the generation
        // and we drop the bodies that got cached as "no frames" while that file
        // was still downloading, so they re-resolve dressed. See the self-heal
        // block at the top of GetAnimationFrames.
        private int _seenAnimFilesGen;
        ...
        )
        {
            ...
            // ── Self-heal background-deferred anim files (big-file legacy shards) ──
            // On shards like Ultima Memento the giant anim2-5.mul stream in AFTER
            // world-entry. Poll for any that have now landed (opens them + bumps
            // the loader generation). When the generation changed, a deferred file
            // arrived: drop the negative-cached bodies — those whose IndexAnimation
            // was built while their .mul was absent, so GetIndices returned Empty
            // and left BOTH Groups and UopGroups null (permanent naked/missing).
            // Nulling those slots forces a fresh GetIndices on the next access,
            // which now finds the file and resolves real frames → bodies pop in
            // dressed. Surgical: only the broken (both-null) entries are cleared,
            // so good UOP/already-loaded bodies never churn the texture atlas.
            _animationLoader.PollDeferredFiles();
            int animGen = _animationLoader.FilesGeneration;
            if (animGen != _seenAnimFilesGen)
            {
                lock (_dataIndexLock)
                {
                    if (animGen != _seenAnimFilesGen)
                    {
                        var _swReclear = System.Diagnostics.Stopwatch.StartNew();
                        int cleared = 0;
                        for (int i = 0; i < _dataIndex.Length; i++)
                        {
                            IndexAnimation e = _dataIndex[i];
                            if (e != null && e.Groups == null && e.UopGroups == null)
                            {
                                _dataIndex[i] = null;
                                cleared++;
                            }
                        }
                        _seenAnimFilesGen = animGen;
                        // v0.8.15 INSTRUMENTATION: the reclear loop only nulls slots (cheap);
                        // the real post-reclear cost is the per-body re-resolution that the
                        // nulled slots trigger on next access ([anim-load] below). Timing both
                        // attributes the 4 post-reclear spikes (#2-5 of the 5 streaming hitches).
                        ClassicUO.Utility.Logging.Log.Trace($"[anim-defer] deferred file landed (gen {animGen}) — recleared {cleared} unresolved bodies for re-render (scanned {_dataIndex.Length} slots in {_swReclear.Elapsed.TotalMilliseconds:F1}ms)");
                    }
                }
            }
            ...
            if (animDir.FrameCount <= 0 && animDir.SpriteInfos == null)
            {
                ...
                try
                {
                    ...
                    // v0.8.15 INSTRUMENTATION (pure timing, no logic): this cold-load
                    // branch runs once per (id,action,dir). After a deferred-file reclear
                    // the recleared bodies re-enter here; time the frame DECODE (read,
                    // CPU on deputy) vs the atlas UPLOAD (GL, proxied to main) separately
                    // so v0.8.16 fixes the right mechanism. Logged below as [anim-load].
                    var _swRead = System.Diagnostics.Stopwatch.StartNew();
                    if (useUOP)
                {
                ...
                double _readMs = _swRead.Elapsed.TotalMilliseconds;
                ...
                var _swUp = System.Diagnostics.Stopwatch.StartNew();
                for (int i = 0; i < frames.Length; i++)
                {
                ...
                double _upMs = _swUp.Elapsed.TotalMilliseconds;
                if (_readMs + _upMs >= 2.0)
                    ClassicUO.Utility.Logging.Log.Trace($"[anim-load] id={id} frames={frames.Length} read={_readMs:F1}ms upload={_upMs:F1}ms gen={_seenAnimFilesGen}");
                }
                finally
                {
