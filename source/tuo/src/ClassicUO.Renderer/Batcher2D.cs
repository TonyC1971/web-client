...
namespace ClassicUO.Renderer
{
    public sealed unsafe class UltimaBatcher2D : IDisposable, IFontStashRenderer
    {
        ...
        // Chunk-mesh renderer (ported from CUO): a resizable index buffer for
        // mesh draws + a flag so the per-frame WorldMatrix=Identity reset is
        // skipped while a world offset is active (else the mesh draws at 0,0).
        ...
        // v0.7.9 WASM port: Brighlight is null on the simplified wasm
        // shader (see Effects/BasicUOEffect.cs). Use `?.SetValue` so the
        // call no-ops on wasm without throwing NullRef.
        public void SetBrightlight(float f) => _basicUOEffect.Brighlight?.SetValue(f);

        // Sprite smoothing (texel-AA, operator goal 2026-07-10): width 0..1
        // from the options slider; mode 0=off 1=silhouette 2=full. GameScene
        // applies it per frame from the profile; the shader gates gumps/text.
        // DrawTexSize uploads ride TazUO's existing per-texture TexelSize
        // block in InternalDraw (unconditional there), so no cache here.
        ...
        private void ApplyStates()
        {
            ...
            // Chunk-mesh: don't reset WorldMatrix while a world offset is active
            // (the mesh draw brackets its calls with SetWorldOffset/ResetWorldOffset).
        ...
        // ── Chunk-mesh renderer draw support (ported from CUO Batcher2D:1206-1272) ──
        // A resizable index buffer + a direct indexed draw that consumes a
        // MeshLayer's GPU-resident vertex buffer, plus world-offset bracketing so
        // the chunk mesh (built in world coords, reused across frames) renders at
        // the right screen position as the camera moves.
        ...
        public void DrawDirectIndexed(Texture2D texture, int startIndex, int primitiveCount, int numVertices)
        {
            ...
            _basicUOEffect.DrawTexSize?.SetValue(new Vector2(texture.Width, texture.Height));
            _basicUOEffect.Pass.Apply();
        ...
        private void InternalDraw(Texture texture, int baseSprite, int batchSize)
        {
            ...
            {
                // v0.7.9 WASM port: TexelSize is null on the simplified
                // wasm shader (see Effects/BasicUOEffect.cs). Use
                // `?.SetValue` so the call no-ops on wasm.
                _basicUOEffect.TexelSize?.SetValue(new Vector2(1f / tex2d.Width, 1f / tex2d.Height));
                // Sprite smoothing rides the same per-texture upload (wasm shader).
                _basicUOEffect.DrawTexSize?.SetValue(new Vector2(tex2d.Width, tex2d.Height));
                _basicUOEffect.Pass.Apply();
            }
        ...
        // Made public (was private) so the ported MeshLayer (chunk-mesh renderer)
        // can hold a PositionNormalTextureColor4[] vertex buffer. Matches CUO.
    ...
    public partial class Resources
    {
        ...
        // v0.7.9: Simplified wasm variant — see source/cuo/.../shaders/
        // IsometricWorld.wasm.fx and BasicUOEffect.cs for the rationale.
        // Same vertex layout as the desktop .fxc so the C# batcher needs
        // no changes; the pixel shader drops hue / spectral / lighting to
        // dodge a MojoShader glsles3 transpile bug that writes zero
        // fragments on WebGL2. Without this variant TUO traps with
        // `unreachable` on the first SetScene Texture2D bind.
        ...
        // task #163: xBRZ freescale upscaler — same batcher vertex frame as
        // xBR.fx, ported from libretro (GPL-2.0+ DeSmuME / MIT Hyllian, see
        // the .fx header). Wired as PostProcessingType.Xbrz.
