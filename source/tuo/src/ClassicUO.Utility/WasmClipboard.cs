// Browser clipboard bridge for the WASM build (ported 1:1 from the cuo fork —
// the classic TUO-web-only gap: cuo routes every clipboard call through these
// shims under BROWSER_WASM while the TazUO fork still hit the SDL clipboard
// P/Invoke, whose unresolved wasm symbol traps the runtime. Player report
// 2026-07-18: chat → Ctrl+A → Ctrl+X = RuntimeError: unreachable).
//
...
//
// Both P/Invokes use only blittable `byte[]` + `int` parameters — no string
// marshalling, no `unsafe`. UTF-8 encoding is done in managed code; the C side
// never sees a C# string. On desktop (no BROWSER_WASM) every method is a
// no-op; callers keep their direct SDL clipboard path behind their own #if.
