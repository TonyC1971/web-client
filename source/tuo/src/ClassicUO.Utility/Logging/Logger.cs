    ...
    public class Logger
    {
        ...
        private void SetLogger(LogTypes type, string text)
        {
            ...
            if ((LogTypes & type) == type)
            {
                ...
                {
#if BROWSER_WASM
                    // v0.7.9: Console.ForegroundColor throws
                    // PlatformNotSupportedException in Mercury MT (no real
                    // terminal). Emit a plain timestamped line — the
                    // browser dev console doesn't render ANSI colour
                    // codes anyway, and Bootstrap.Boot()'s first log call
                    // crashed the entire WASM boot pre-fix. Mirrors how
                    // CUO upstream-fork handles the same call site.
