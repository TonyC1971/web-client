...
namespace ClassicUO.Assets
{
    public sealed class SoundsLoader : UOFileLoader
    {
        ...
        {
#if BROWSER_WASM
            // v0.7.9 (mirror of CUO v0.3.33): under WASM, GetTrueFileName has
            // no useful work to do. Music .mp3 files live in the nginx-served
            // gamefile tree and are NOT mounted into MEMFS. Directory.GetFiles
            // on the lazy MEMFS mount throws DirectoryNotFoundException —
            // escaping all the way up through TryParseConfigLine + the outer
            // try-catch in Load, derailing the whole AssetServer init. The
