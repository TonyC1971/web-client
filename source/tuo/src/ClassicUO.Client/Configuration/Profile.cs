...
namespace ClassicUO.Configuration
{
    ...
    public sealed partial class Profile : INotifyPropertyChanged
    {
        ...
        public bool UseAlternativeLights { get; set => SetProperty(ref field, value); } = true; // ON by default for new profiles (operator request)
        ...
        // slider-driven, never breaks sprites. Level 0..100 (0 = off); Full
        // switches silhouette-only AA to per-tap hue-resolved smoothing.
        public int SpriteSmoothingLevel { get; set => SetProperty(ref field, value); } = 0;
        public bool SpriteSmoothingFull { get; set => SetProperty(ref field, value); } = false;
        // Sprite-smoothing ALGORITHM (parity with CUO): 0=Off, 1=Silhouette AA,
        // 2=Full AA, 3=xBR, 4=xBRZ. -1 = legacy/auto (derive from Level + Full so old
        // profiles keep behaving). The TUO renderer already ships XBREffect/XBRZEffect,
        // so all five modes are real. (operator 2026-07-23)
        public int SpriteSmoothingMode { get; set => SetProperty(ref field, value); } = -1;

        // v0.9.516 (operator 2026-07-28): sprite smoothing goes back to OFF for
        // EVERYONE, once. Anyone who had it on must deliberately turn it on again.
        // This marker is what makes it a one-shot migration instead of a setting the
        // player can never keep: without it the reset would fire on every load.
        public bool SmoothingResetV2 { get; set => SetProperty(ref field, value); } = false;

        // Resolve the effective 0..4 sprite-smoothing mode (parity with CUO's unified
        // 5-mode selector). SpriteSmoothingMode < 0 = legacy/auto: a pre-unified profile
        // that picked xBR/xBRZ through the old standalone "Processing type" combo is
        // surfaced as mode 3/4; otherwise derive from Level + Full. Shared by GameScene
        // (render) and ModernOptionsGump (UI) so both agree. (operator 2026-07-23)
        public int ResolveSpriteSmoothingMode()
        {
            int mode = SpriteSmoothingMode;
            if (mode < 0)
            {
                if (EnablePostProcessingEffects && PostProcessingType == 3) { mode = 3; }
                else if (EnablePostProcessingEffects && PostProcessingType == 4) { mode = 4; }
                else { mode = SpriteSmoothingLevel <= 0 ? 0 : (SpriteSmoothingFull ? 2 : 1); }
            }
            return mode < 0 ? 0 : (mode > 4 ? 4 : mode);
        }
        ...
        private void LoadMyraControl(XmlElement xml)
        {
            ...
            switch (type)
            {
                ...
                case "ClassicUO.Game.UI.MyraWindows.AssistantWindow":
                    ...
                    break;
#if !BROWSER_WASM
                // Scripting UI windows stripped in the WASM build —
                // their backing types (LegionScripting etc.) are out
                // of scope. Profile XML still loads; these cases just
                // become no-ops (the default branch logs an error).
