...
namespace ClassicUO.Game.GameObjects
{
    public partial class Mobile
    {
        ...
        // ── [naked-diag] on-world body+equipment resolution tracer (PLAYER only) ──
        // Occlusion-immune diagnosis of the "dressed paperdoll / naked on-world char"
        // bug on big-file legacy shards (Ultima Memento). Logs, per player body and
        // per equipment layer, the render OUTCOME — OK / NOFRAMES (frames.Length==0,
        // i.e. anim file absent or graphic not present) / NOTEX (frame exists but the
        // texture failed to decode/upload) — and ONLY on a state TRANSITION, so the
        // naked→dressed timing as deferred anim2-5.mul land is visible with no
        // per-frame spam. Debug-gated through Log.Trace (silent unless dev=2) and
        // bounded by a hard emit cap so a flickering layer can never flood console.
        private static readonly Dictionary<long, byte> _nakedDiagState = new Dictionary<long, byte>();
        private static int _nakedDiagEmitted;
        private const int NAKED_DIAG_CAP = 800;

        private static void NakedDiag(Mobile owner, Item entity, bool isEquip, ushort id, byte animGroup, byte dir, int framesLen, bool texNull)
        {
            if (owner == null || !owner.IsPlayer || _nakedDiagEmitted >= NAKED_DIAG_CAP)
            {
                return;
            }

            byte outcome = framesLen == 0 ? (byte)1 : (texNull ? (byte)2 : (byte)0); // 0=OK 1=NOFRAMES 2=NOTEX
            int kindCode = isEquip ? 1 : 0;
            long key = ((long)id << 24) | ((long)animGroup << 16) | ((long)dir << 8) | (uint)kindCode;

            if (_nakedDiagState.TryGetValue(key, out byte prev) && prev == outcome)
            {
                return; // unchanged state → don't re-log
            }
            _nakedDiagState[key] = outcome;
            _nakedDiagEmitted++;

            string kind = isEquip ? "equip" : "body";
            string state = outcome == 0 ? "OK" : (outcome == 1 ? "NOFRAMES(frames.Length==0)" : "NOTEX(Texture==null)");
            ushort itemGraphic = entity != null ? entity.Graphic : (ushort)0;
            ClassicUO.Utility.Logging.Log.Trace(
                $"[naked-diag] PLAYER {kind} animID=0x{id:X4} item=0x{itemGraphic:X4} grp={animGroup} dir={dir} frames={framesLen} -> {state}");
        }
        ...
        )
        {
            ...
            if (frames.Length == 0)
            {
                if (!hasShadow && !isMount) NakedDiag(owner, entity, isEquip, id, animGroup, dir, 0, true);
            ...
            if (!hasShadow && !isMount) NakedDiag(owner, entity, isEquip, id, animGroup, dir, frames.Length, spriteInfo.Texture == null);
