...
#if BROWSER_WASM
        // Chunk-mesh renderer hooks (ported from CUO v0.5.x perf track). When a
        // Land/Static/Multi tile is baked into its chunk's batched ChunkMesh,
        // InChunkMesh=true and MeshSpriteIndex points at its slot in the
        // MeshLayer's vertex buffer; AddTileToRenderList then toggles visibility
        // instead of re-pushing the sprite to the per-frame immediate-draw queue.
