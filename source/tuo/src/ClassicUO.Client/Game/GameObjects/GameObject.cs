...
namespace ClassicUO.Game.GameObjects
{
    ...
    public abstract partial class GameObject : BaseGameObject
    {
        ...
        public void SetInWorldTile(ushort x, ushort y, sbyte z)
        {
            ...
            Z = z;
#if BROWSER_WASM
            if (CUOEnviroment.Debug) ClassicUO.Utility.Logging.Log.Trace($"[swt-debug] SWT1: pre-UpdateScreenPosition ({x},{y},{z})");
#endif
            UpdateScreenPosition();
#if BROWSER_WASM
            if (CUOEnviroment.Debug) ClassicUO.Utility.Logging.Log.Trace("[swt-debug] SWT2: post-UpdateScreenPosition pre-AddToTile");
#endif
            AddToTile(x, y);
#if BROWSER_WASM
            if (CUOEnviroment.Debug) ClassicUO.Utility.Logging.Log.Trace("[swt-debug] SWT3: post-AddToTile");
#endif
        }
