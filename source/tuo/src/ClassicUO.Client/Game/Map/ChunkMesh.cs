...
namespace ClassicUO.Game.Map
{
    ...
    // public (CUO has it internal, but TUO's Chunk is a PUBLIC class so its
    // `public readonly ChunkMesh Mesh` field requires ChunkMesh be public too).
...
#if BROWSER_WASM
        // v0.4.3 dirty-mark tracking. The v0.4.2 chunks-diff data showed
        // ~9 chunks dirty per heavy fill at running speed even though
        // entered=0 (those chunks never left the viewport). Something is
        // calling MarkDirtyIfNeeded on already-visible chunks every frame.
        // These counters get sampled by GameScene.FillGameObjectList at
        // end of fill so we can attribute the marks to a category.
        public static int LandMarks = 0;
        public static int StaticMarks = 0;
        public static int MultiMarks = 0;
        public static int TransitionsToDirty = 0;

        // v0.4.4 mesh.Build profiling. Sum the time spent in Build()
        // per fill so we can see the individual cost vs deferred count.
        public static long BuildTotalMs = 0;
        public static int BuildCalls = 0;
        public static int SoftClearCalls = 0;
#endif
...
#if BROWSER_WASM
                TransitionsToDirty++;
                if (obj is GameObjects.Land) LandMarks++;
                else if (obj is Static) StaticMarks++;
                else if (obj is Multi) MultiMarks++;
#endif
            }
        }
...
#if BROWSER_WASM
            // Bug O10: AnimatedWaterEffect excludes wet tiles from the
            // GPU mesh so LandView.cs CPU animation can take over. On
            // wasm ALL lands go through the mesh unconditionally
            // (LandView.Draw is never reached), so honouring the
            // profile flag would silently delete all water tiles. Pin
            // to false until the CPU-animation path routes through
            // the mesh pipeline.
            _animatedWaterEffect = false;
#else
...
#if BROWSER_WASM
            BuildTotalMs += __buildSw.ElapsedMilliseconds;
            BuildCalls++;
#endif
        }
