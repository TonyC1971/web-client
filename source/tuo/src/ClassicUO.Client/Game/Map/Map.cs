...
namespace ClassicUO.Game.Map
{
    public sealed class Map
    {
        ...
        {
#if BROWSER_WASM
            // v0.8.6 ROOT-FIX (defense): NEVER load chunks on a Task.Run worker
            // under Mercury .NET-10 MT. A second allocating worker GC-safepoint-
            // races the ReceiveLoopAsync worker → "WAITING for N threads" worker
            // trap (Uncaught [object WebAssembly.Exception]) → black screen. Run
            // synchronously on the calling (main) thread — CUO parity. Callers
            // (GameScene FillGameObjectList, main thread) already tolerate the
            // 1-frame delay (chunk is placed by ProcessLoadedChunks next frame).
            // GameScene.ASyncMapLoading is forced false under WASM so this path
            // is normally unused; this guard makes the worker trap impossible
            // even if the flag is flipped at runtime (e.g. via a synced profile).
            try
            {
...
#if BROWSER_WASM
        // v0.7.9 iter 51 (port from CUO v0.5.21 — Map.cs:278-307):
        // GetTileZ MapBlock LRU cache.
        //
        // Without this, the first Chunk.Load on WASM does 64 tiles ×
        // 11 GetTileZ calls (Land.ApplyStretch neighbours) = 704
        // fresh `MapFile.ReadAt<MapBlock>` calls, each stackallocing
        // ~196 bytes + Stream.Read. That allocation burst trips a
        // Mercury MT GC stop-the-world that races with the BG
        // ReceiveLoopAsync worker → "WAITING for 1 threads, got 0
        // suspended" → mono-threads.c:336 <disabled> →
        // WebAssembly.Exception trap. iter 50 fixed the parallel
        // allocation OOB inside Chunk.Load; this iter 51 fixes the
        // GetTileZ allocation storm. CUO's measurement showed 85 %
        // cache hit rate on a typical chunk fill.
        ...
        public sbyte GetTileZ(int x, int y)
        {
            ...
            ref IndexMap blockIndex = ref GetIndex(blockX, blockY);
            ...
            // v0.7.9 iter 57: same MapBlock read fix as Chunk.Load — read
            // raw bytes + MemoryMarshal.Read instead of Read<MapBlock>()'s
            // `&v + sizeof` path, which traps on the InlineArray struct
            // under Mercury MT AOT.
            const int MAPBLOCK_BYTES = 196;
            Span<byte> mbBuf = stackalloc byte[MAPBLOCK_BYTES];
            blockIndex.MapFile.ReadAt((long)blockIndex.MapAddress, mbBuf);
            MapBlock block = System.Runtime.InteropServices.MemoryMarshal.Read<MapBlock>(mbBuf);
...
#if BROWSER_WASM
        /// <summary>
        /// Async pre-fetch of the asset-worker's server-baked worldmap PNG.
        /// MUST be awaited from an async caller (WorldMapGump) BEFORE
        /// GenerateMapPng — a SYNCHRONOUS block on the browser fetch traps the
        /// WASM runtime (uncatchable abort, not a C# exception). On a 200 it
        /// writes the PNG into the cache under the .mul-size fingerprint name,
        /// so GenerateMapPng's prebaked-check finds it and skips the 30-90s
        /// in-browser render. Ported from CUO WorldMapGump.PrefetchMapPngAsync.
        /// </summary>
        public static async Task PrefetchMapPngAsync(int mapIndex)
        {
            try
            {
                ...
                // CUO_SHARD_BASE (e.g. "server-4") + CUO_HTTP_ORIGIN (e.g.
                // "https://uonexus.com") are injected by main.js via the .NET
                // boot-config env. The URL must be ABSOLUTE — .NET's HttpClient
                // rejects a relative request URI with no BaseAddress.
                string shardBase = Environment.GetEnvironmentVariable("CUO_SHARD_BASE");
                string origin = Environment.GetEnvironmentVariable("CUO_HTTP_ORIGIN");
                if (string.IsNullOrEmpty(shardBase) || string.IsNullOrEmpty(origin))
                    return;
                ...
                // Same fingerprint GenerateMapPng's prebaked-check looks for and
                // the asset-worker bakes: the .mul byte sizes.
                string pngName = $"map{mapIndex}_sz_{mapFile.Length}_{staticFile.Length}.png";
                string outPath = Path.Combine(_mapsCachePath, pngName);
                if (File.Exists(outPath))
                    return;

                string url = $"{origin}/{shardBase}/MapsCache/{pngName}";
                using var http = new System.Net.Http.HttpClient();
                http.Timeout = TimeSpan.FromSeconds(60);
                using var resp = await http.GetAsync(url).ConfigureAwait(false);
                if (!resp.IsSuccessStatusCode)
                {
                    Log.Trace($"WorldMap: server PNG unavailable (HTTP {(int)resp.StatusCode}) {url} — rebuilding locally");
                    return;
                }
                ...
                Log.Trace($"WorldMap: prefetched server PNG ({bytes.Length} B) {url}");
            }
            catch (Exception ex)
            {
                // Any failure → caller's GenerateMapPng falls back to the local render.
                Log.Trace($"WorldMap: PNG prefetch failed ({ex.Message}) — rebuilding locally");
            }
        }
#endif
...
#if BROWSER_WASM
            // Prefer the asset-worker's prebaked worldmap PNG when WorldMapGump's
            // prefetch (Map.PrefetchMapPngAsync) has dropped it into the cache —
            // named by the .mul byte sizes, matching the server bake — so we skip
            // the 30-90s in-browser render entirely. Falls through to the local
            // render if absent (prefetch missed / shard has no baked PNG).
            try
            {
                var _pmf = Client.Game.UO.FileManager.Maps.GetMapFile(mapIndex);
                var _psf = Client.Game.UO.FileManager.Maps.GetStaticFile(mapIndex);
                if (_pmf != null && _psf != null)
                {
                    string _prebaked = Path.Combine(_mapsCachePath, $"map{mapIndex}_sz_{_pmf.Length}_{_psf.Length}.png");
                    if (File.Exists(_prebaked))
                        return _prebaked;
                }
            }
            catch { /* fall through to the local render below */ }
#endif
