    ...
    public sealed class UltimaLive
    {
        ...
        public class ULMapLoader : MapLoader
        {
            ...
            public void CheckForShardMapFile(int mapId)
            {
                ...
                // BLACK-WORLD seed fix (UltimaLive ultra-customized shards, e.g. Ultima
                // Adventures). The shadow map/statics/staidx files this loader reads MUST be
                // seeded from the REAL files that were downloaded and mounted at /uo/. The old
                // code asked GetMapFile(mapId) whether a loaded map existed, but that reads
                // _currentMapFiles, which is still all-null here: Enable() calls
                // CheckForShardMapFile at the 0x01 case BEFORE this loader's Load() runs, so
                // the lookup always returned null → CreateNewPersistentMap wrote a BLANK
                // map+staidx+statics. Worse, once those blank files existed the statics/staidx
                // copies were skipped, so the entire facet rendered BLACK (only server
                // live-edits ever streamed terrain in). On WASM the shadow lives in ephemeral
                // MEMFS under '/' (no OPFS/IDBFS mount), so it is re-seeded fresh every page
                // load — there is no stale blank shadow to worry about, and UltimaLive
                // re-streams its live edits on every login so an ephemeral seed is correct.
                //
                // A facet is "real" only when the full triplet (map+staidx+statics) is present
                // on disk, which is how standard .mul facets ship (adventures downloads
                // map0-5/statics0-5/staidx0-5.mul). Seed all three from /uo/ and the terrain +
                // statics render immediately; the server then streams its live edits on top.
                // Anything without a real base (a server-defined custom facet like map33/34, or
                // a .uop-only map with no .mul we can copy) falls back to a fully blank facet
                // the server streams into. Each CopyFile is guarded by File.Exists on BOTH the
                // source (CopyFile itself does not guard the source → would throw) and the
                // destination (skip if the shadow already exists this session).
                bool haveRealTriplet = File.Exists(oldMap) && File.Exists(oldStatics) && File.Exists(oldStaIdx);
            ...
            public unsafe void ReloadBlock(int map, int blockNumber)
            {
                ...
                // .NET 10 Mercury MT WASM AOT crash fix (same class as the v0.8.10
                // minimap fix): ReadAt<StaidxBlock> is a generic-VIRTUAL method over
                // a value type. A generic-virtual call cannot be AOT-specialized, so
                // under Mercury MT it dispatches through gsharedvt and traps with
                // "function signature mismatch" — THE crash on UltimaLive shards
                // (e.g. Ultima Adventures) the instant the server streams a statics
                // block (OnUltimaLivePacket -> ReloadBlock). The NON-generic
                // ReadAt(long, Span<byte>) overload is virtual but NOT generic (a
                // normal vtable call, AOT-safe) and on ULFileMul it takes the _ioLock,
                // so a concurrent live-statics WriteArray can't tear the read. Then
                // MemoryMarshal the 12-byte POD struct out (StaidxBlock has no
                // InlineArray). Mirrors the Chunk/Map WASM map-read path; desktop
                // keeps the generic read.
#if BROWSER_WASM
                Span<byte> stIdxBuf = stackalloc byte[staticidxblocksize];
                fileidx.ReadAt(block * staticidxblocksize, stIdxBuf);
                StaidxBlock st = System.Runtime.InteropServices.MemoryMarshal.Read<StaidxBlock>(stIdxBuf);
#else
