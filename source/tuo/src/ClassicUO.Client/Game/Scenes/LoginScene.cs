...
#if BROWSER_WASM
            // Tell main.js the LoginGump is up so the "Starting up..." loader
            // dismisses immediately instead of waiting out its 15s safety timeout.
            // CUO emits this at LoginScene.cs:100; TUO was missing it, so the
            // loader lingered the full 15s while LoginScene.Load() already ran
            // (and PlayMusic below fired) BEHIND the still-visible loader — which
            // is why the login music was heard during "Starting up...". Non-blocking
            // (WasmSignal.Send -> wasm_signal_event_async -> MAIN_THREAD_ASYNC_EM_ASM);
            // main.js's `cuo:login-gump-added` listener calls hideLoaderOnce().
            // Guarded on _currentGump so the async IP/port InputRequest paths (which
            // add the gump later in a callback) don't fire it prematurely; the
            // webclient always has the IP pre-set by the proxy, so the else-branch
            // above runs synchronously and _currentGump is non-null here.
