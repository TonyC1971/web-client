...
namespace ClassicUO.Game.Scenes
{
    public partial class GameScene
    {
...
#if BROWSER_WASM
        // Chunk-mesh renderer: the chunk currently being processed in the fill
        // loop (so PushToRenderList can divert meshed tiles to its mesh), and the
        // visible chunks this frame (so DrawWorld can draw their batched meshes).
        private Chunk _currentMeshChunk;
...
#if BROWSER_WASM
            // ── Chunk-mesh divert ────────────────────────────────────────────
            // If this Land/Static/Multi was baked into the current chunk's batched
            // mesh, render it via a GPU visibility toggle instead of adding it to
            // the per-frame immediate-draw list. Meshed objects therefore never
            // enter _renderList* → no double-draw, no View.Draw skip needed. All of
            // TUO's per-type alpha/cull/foliage logic above already ran and set
            // obj.AlphaHue, which we reuse here.
            if (obj.InChunkMesh && obj.MeshSpriteIndex >= 0 && _currentMeshChunk != null)
            {
                // Land: promote a fresh AlphaHue 0 → opaque (else mesh land draws
                // black until the next alpha tick — CUO does the same).
                if (obj is Land && obj.AlphaHue == 0)
                {
                    obj.AlphaHue = 0xFF;
                }
                ...
                var meshLayer = obj is Land
                    ? _currentMeshChunk.Mesh.Land
                    : _currentMeshChunk.Mesh.Statics;
                meshLayer.SetVisible(obj.MeshSpriteIndex, obj.AlphaHue);
                ApplyMeshHue(obj, meshLayer);

                // Keep mouse-selection working for meshed tiles.
                if (allowSelection && obj.Z <= _maxGroundZ && obj.AllowedToDraw && obj.CheckMouseSelection())
                {
                    ...
                    {
                        if (obj.CalculateDepthZ() >= prevSel.CalculateDepthZ())
                        {
...
#if BROWSER_WASM
        // ── Chunk-mesh renderer helpers (ported from CUO) ────────────────────
        // ApplyMeshHue: CUO GameSceneDrawingSorting.cs:666-718. Writes the
        // hue (x) + shader-mode (y) for a meshed Land/Static/Multi sprite, so
        // mouse-over highlight / out-of-range / dead effects work for meshed
        // tiles (which never hit the per-object MobileView/ItemView hue path).
        private void ApplyMeshHue(GameObject obj, ClassicUO.Renderer.MeshLayer layer)
        {
            ...
            {
                hueX = hue - 1;
                if (obj is Land landH && landH.IsStretched)
            ...
            {
                hueX = 0;
                if (obj is Land landN && landN.IsStretched)
        ...
        // DrawMeshLayer: CUO RenderLists.cs:188-208. Draws one chunk's batched
        // mesh layer from its GPU-resident vertex buffer with per-frame
        // visibility filtering (caller brackets with SetWorldOffset).
        private static int DrawMeshLayer(UltimaBatcher2D batcher, ClassicUO.Renderer.MeshLayer layer)
        {
