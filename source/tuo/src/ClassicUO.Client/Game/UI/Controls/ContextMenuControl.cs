...
namespace ClassicUO.Game.UI.Controls
{
    ...
    public class ContextMenuShowMenu : Gump
    {
        ...
        public ContextMenuShowMenu(World world, List<ContextMenuItemEntry> list) : base(world, 0, 0)
        {
            ...
            // T3-1 (2026-07-20): cap at nearly the FULL window instead of half — the
            // half cap hid the world map's Marker manager below a near-invisible
            // scroll. Uses upstream's logical-space windowHeight.
            int _menuCap = windowHeight - 40;
            if (_menuCap < 160) { _menuCap = 160; }
            if (y >= _menuCap)
            {
                y = _menuCap;
            }
