...
namespace ClassicUO.Game.UI.Gumps
{
    ...
    public class ModernOptionsGump : BaseOptionsGump
    {
        ...
        // 900x700 is TazUO's desktop design size. On a laptop viewport (and on web,
        // where the canvas IS the window) that overflows the screen on every side, so
        // the panel is unusable exactly where space is tightest — operator 2026-07-26:
        // "en resoluciones de portatil desborda por todos lados". Clamp to the logical
        // window with a margin: the gump keeps its design size whenever it FITS (so
        // nothing changes on a desktop) and shrinks to fit when it does not. The layout
        // is built from Width/Height (see BaseOptionsGump.Build), and the right-hand
        // pane is a scroll area, so a shorter box scrolls instead of clipping.
        private const int DESIGN_W = 900, DESIGN_H = 700;
        private const int MIN_W = 520, MIN_H = 360;   // below this the two columns stop making sense
        private static int FitWidth()
        {
            int avail = ScaleHelper.LogicalWindowWidth - 24;   // 12px breathing room per side
            return avail >= DESIGN_W ? DESIGN_W : Math.Max(MIN_W, avail);
        }
        private static int FitHeight()
        {
            int avail = ScaleHelper.LogicalWindowHeight - 24;
            return avail >= DESIGN_H ? DESIGN_H : Math.Max(MIN_H, avail);
        }

        public ModernOptionsGump(World world) : base(world, FitWidth(), FitHeight(),
        ...
        private void BuildVideo()
        {
            ...
            // The standalone "Enable post processing effects" + "Texture filter" controls
            // were removed (operator 2026-07-24). xBR/xBRZ are driven by the unified Sprite
            // smoothing selector below; keeping a SECOND control over the same post-fx
            // pipeline created confusing overlaps (a Texture filter that xBR silently
            // ignored, and xBR running while post-processing was toggled "off"), which bit
            // once the rail exposed the smoothing selector too. SpriteSmoothingMode is now
            // the ONLY control (GameScene.ResolveEffectiveFilterMode), matching CUO. The
            // point/linear/anisotropic full-screen samplers (niche in a pixel client) are
            // gone with it. See docs/WEB_FORK_ADAPTATIONS.md §3.5.
            ...
            #region Sprite smoothing

            // Its OWN subcategory (operator 2026-07-25: "las opciones de sprite smoothing
            // deben estar en un submenu aparte en ambos menus de opciones de ambos
            // clientes"). CUO already ships a dedicated "Sprite Smoothing" options page;
            // on TUO these two controls used to be tacked onto the END of Shadows, which
            // is neither discoverable nor symmetric with CUO.
            page = ((int)PAGE.Video + 1005);
            content.AddToLeft(SubCategoryButton("Sprite smoothing", page, content.LeftWidth));
            content.ResetRightSide();

            // Sprite smoothing — full CUO parity (operator 2026-07-23): the same 5-mode
            // selector (Off / Silhouette AA / Full AA / xBR / xBRZ) plus an AA-strength
            // slider. Lambdas write the profile directly and GameScene reads it per frame
            // via ResolveSpriteSmoothingMode, so both controls are HOT. Legacy profiles
            // (SpriteSmoothingMode == -1) resolve to the equivalent mode on first display.
            // Literal strings: webclient-specific knobs with no upstream lang entry; the
            // product UI is English-only.
            int _smMode = profile.ResolveSpriteSmoothingMode();

            // Performance warning ABOVE the selector (operator 2026-07-28: players on
            // older GPUs reported heavy lag with this on). It is off by default again,
            // and this is what they read before choosing to turn it back on.
            content.AddToRight
            (
                TextBox.GetOne
                (
                    "WARNING: smoothing costs GPU time. On older or integrated graphics it "
                    + "can drop the frame rate hard, especially the xBR / xBRZ upscale modes. "
                    + "If the game stutters, set this back to Off.",
                    ThemeSettings.FONT, ThemeSettings.STANDARD_TEXT_SIZE, ThemeSettings.TEXT_FONT_COLOR,
                    TextBox.RTLOptions.Default(ThemeSettings.COMBO_BOX_WIDTH + 60)
                ), true, page
            );
            content.BlankLine();
            ...
            (
                new ComboBoxWithLabel
                (World,
                    "Smoothing mode", 0, ThemeSettings.COMBO_BOX_WIDTH,
                    new string[]
                    {
                        "Off (sharp pixels)", "Silhouette AA", "Full AA",
                        "xBR upscale (GPU)", "xBRZ upscale (GPU)"
                    },
                    _smMode,
                    (s, n) =>
                    {
                        profile.SpriteSmoothingMode = s;
                        // Modes 3/4 drive the full-screen xBR/xBRZ post-pass; refresh it
                        // now so the change is instant (GameScene also recomputes per frame).
            ...
            (
                new SliderWithLabel
                (
                    "AA strength (Silhouette/Full)", 0, ThemeSettings.SLIDER_WIDTH,
                    0, 100,
                    profile.SpriteSmoothingLevel, (r) => { profile.SpriteSmoothingLevel = r; }
                ), true, page
            );
