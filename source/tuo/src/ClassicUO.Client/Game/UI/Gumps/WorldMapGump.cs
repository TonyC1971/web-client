...
public class WorldMapGump : ResizableGump
{
    ...
    private void SetTopMost(bool value, bool save)
    {
        ...
        // The frame is part of the window, not a side effect of layering. TazUO tied
        // it to TopMost -- and inverted relative to CUO, which uses !_isTopMost -- so
        // with the flag off (the default) the map rendered with NO border at all
        // (operator 2026-07-27).
        ShowBorder = true;

        // Not-top-most must mean "an ordinary window", not "pinned underneath
        // everything". UILayer.Under kept the map below every other gump no matter
        // what you clicked, which is why the chat bar painted over it. Default lets
        // it raise on click like any other window; TopMost still forces it above.
...
#if !BROWSER_WASM
        // Desktop-only entries (audit T3-1): the map web server is an
        // HttpListener — impossible in the browser build. The crash was already
        // guarded (Start() catches, OpenWorldMapWebWindow prints "desktop-only"),
        // but the entries still RENDERED in the web menu as dead weight. Hidden
        // symmetrically with their ContextMenu.Add sites below.
    ...
    private void BuildContextMenu()
    {
        ...
        ContextMenu.Add("", null);
#if !BROWSER_WASM
        // Desktop-only web-map entries — hidden in wasm (audit T3-1); their
        // dictionary entries above are guarded symmetrically.
    ...
    public void ChangeMap(int index)
    {
        ...
        {
#if BROWSER_WASM
            // Pre-fetch the asset-worker's baked worldmap PNG before rendering so
            // GenerateMapPng uses it (incl. UltimaLive custom continents like
            // Adventures map33-36) instead of the 30-90s in-browser local render.
            if (_loadingTask is { Status: TaskStatus.Running })
                _loadingTask = _loadingTask.ContinueWith(async _ => { await Map.Map.PrefetchMapPngAsync(index); LoadMap(index, _map, _world); }).Unwrap();
            else
                _loadingTask = Task.Run(async () => { await Map.Map.PrefetchMapPngAsync(index); LoadMap(index, _map, _world); });
#else
            if (_loadingTask is { Status: TaskStatus.Running })
...
#if BROWSER_WASM
            // Pre-fetch the asset-worker's baked worldmap PNG so GenerateMapPng
            // uses it instead of the 30-90s in-browser render (await is safe;
            // a synchronous fetch block would trap the WASM runtime).
            await Map.Map.PrefetchMapPngAsync(mapIndex);
#endif
    ...
    private void DrawAll(UltimaBatcher2D batcher, Rectangle srcRect, int gX, int gY, int halfWidth, int halfHeight)
    {
        ...
        {
            DrawWMEntity(batcher, uoamPeers[ui], gX, gY, halfWidth, halfHeight, Zoom);
        }
    ...
    public override void OnMouseUp(int x, int y, MouseButtonType button)
    {
        ...
        if (button == MouseButtonType.Left && !Keyboard.Alt && !Keyboard.Ctrl && !Keyboard.Shift)
        {
            ...
            {
                ClassicUO.Utility.Clipboard.SetClipboardText($"{World.Player.X}, {World.Player.Y}, {World.Player.Z}");
