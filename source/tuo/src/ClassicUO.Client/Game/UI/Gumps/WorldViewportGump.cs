...
#if !BROWSER_WASM
            // Upstream warns when the UO folder sits inside the TazUO install, because an
            // uninstall could take the game files with it. On the web there IS no UO folder and
            // no uninstall: the files live in a virtual filesystem fetched from the pool, and both
            // paths in this comparison are virtual, so the condition matches and every browser
            // player was told, in red, that they might "accidentally have your UO files deleted".
            // A warning that cannot be true and cannot be acted on is worse than no warning.
            // Caught 2026-08-01 in a production guest-gameview screenshot, not by the compiler —
            // which is the whole reason WEB_FORK_ADAPTATIONS.md exists.
...
#if !BROWSER_WASM
            // v5.3 sync: upstream polls the TazUO Firebase for unvoted community
            // polls right after login. On the web that is a third-party request
            // from our origin (CSP-blocked, privacy-relevant) — skipped.
