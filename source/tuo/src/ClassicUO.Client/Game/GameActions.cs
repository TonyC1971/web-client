...
internal static class GameActions
{
...
#if BROWSER_WASM
        // Web always uses the legacy gump (see OpenSettings below).
    ...
    {
#if BROWSER_WASM
        // WEB: always the LEGACY ModernOptionsGump, never the Myra NewOptionsWindow.
        // v5.3 added UseNewOptionsWindow (default TRUE), so upstream sends every caller
        // to Myra's Options window — and on wasm that window opens with an EMPTY content
        // area (chrome, search box and category list render; the pages do not). Verified
        // in-world 2026-07-25 and against a capture from v0.9.485, so it is not a
        // regression of ours — but it WAS masked while the rail carried its own options
        // panel. That panel is gone (operator 2026-07-25: every client opens its own
        // native window), so this is now the ONLY options UI on TUO web and it has to be
        // the one that actually works. Covers every entry point: the rail gear
        // (RailBridgeApi.OpenNativeOptions), the paperdoll OPTIONS button and macros.
        // Revisit only if Myra windows are ever proven to render their content on wasm.
    ...
    {
#if BROWSER_WASM
        // Browser sandbox forbids spawning a child Process AND opening
        // a local HttpListener on a TCP port. The whole feature is
        // moot under WASM anyway — the client itself is already in a
        // browser tab; users can right-click the in-game world map to
        // copy a deep link. Print a friendly message instead of
        // attempting the operation. Lesson learned from iter 43
        // (Ping field init) — gate WASM-unsafe entry points BEFORE
        // they bite as phantom traps.
        Print(World.Instance, "World map web window is a desktop-only feature.", 0x21);
        return;
#else
