                ...
                if (evalutate)
                {
                    ...
                    if (X != step.X || Y != step.Y)
                    {
                        ...
                        else
                        {
                            ...
                            Offset.Y = (sbyte)y;
#if BROWSER_WASM
                            // v0.4.95 fix kept: mark RealScreenPosition stale
                            // every interpolation frame so the next AddTile
                            // ToRenderList re-projects via the gate at
                            // GameSceneDrawingSorting.cs:918. Necessary but not
                            // sufficient alone — see the commit-side force-
                            // refresh below (v0.4.97). Together they close the
                            // 1-frame backward-snap flicker on OTHER mobiles.
...
#if BROWSER_WASM
                        // v0.4.97: force-refresh RealScreenPosition at the
                        // commit moment using the GameScene's current
                        // DrawOffset. v0.4.96 log analysis proved that
                        // AddTileToRenderList does not always visit the just-
                        // committed mobile on the very next draw frame: the
                        // mobile renders at OLD_RealScreenPosition + Offset
                        // (0,0) for ~27 ms / ~4 frames @ 144 Hz, which the
                        // operator perceives as a 1-frame backwards-snap to
                        // the previous tile position. By calling
                        // UpdateRealScreenPosition here we guarantee the
                        // very first frame after commit reads a fresh
                        // projection. Cost: one extra projection (4 subs +
                        // 2 muls + 1 shift) per step commit per mobile,
                        // ~5 Hz per moving mobile — negligible. Safe to
                        // call even if AddTileToRenderList later refreshes
                        // again because UpdateRealScreenPosition is
                        // idempotent (uses current X/Y/Z and supplied cam).
                        // GameSceneDrawingSorting._offset is read-only
                        // outside GetViewPort, exposed as DrawOffset.
                        if (Client.Game.Scene is Game.Scenes.GameScene gs)
                        {
                            UpdateRealScreenPosition(gs.DrawOffset.X, gs.DrawOffset.Y);
                        }
#endif
                    }
