...
namespace ClassicUO.Game.GameObjects
{
    ...
    internal abstract partial class GameObject
    {
        ...
        // v0.4.20: rolling-union hit-test bbox. FrameInfo is reset to (0,0,0,0)
        // at the start of every Draw() and re-populated only with the bbox of
        // the sprite(s) actually drawn THIS frame. For Mobiles whose anim
        // cycle alternates between frames with different bbox extents (idle
        // horse "breathing" anim being the witness case), pixel-cursor
        // positions that are inside the silhouette on one anim frame can be
        // outside FrameInfo on the next anim frame — so the per-frame hit
        // test rejects ~75% of clicks at a stable mouse position.
        // StableHitBox accumulates the union of all FrameInfo values seen
        // across the most recent anim frames; CheckMouseSelection uses it
        // for the bbox quick-test instead of FrameInfo. Reset whenever the
        // graphic, direction, or world position changes (silhouette
        // genuinely changed). Wasm-only — desktop CUO renders at 60 fps with
        // anim at 40 fps so the off-frame race is masked.
        public Rectangle StableHitBox;
