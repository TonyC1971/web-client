...
namespace ClassicUO.Game
{
    internal static class Constants
    {
        ...
        // 250 → 60 (operator 2026-07-10 "limita todos los clientes a 60 fps max"):
        // in the browser the display loop is rAF-driven anyway (~display Hz) and
        // >60 targets only burn main-thread time the page shares with everything
        // else. SetRefreshRate clamps saved settings on boot and the video
        // options FPS slider takes its range from this constant, so the UI
        // shows the new ceiling automatically.
