...
namespace ClassicUO.Game
{
    internal sealed class GameCursor
    {
        ...
        public void CreateGraphic(float dpiScale) {
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    ...
                    // hotX/hotY are computed by `CreateCursorSurfacePtr`
                    // from the green marker pixel in the cursor art
                    // (Art.cs:125-177) — they're valid EVEN WHEN the
                    // subsequent `SDL_CreateSurfaceFrom` / `SDL_Create
                    // ColorCursor` path fails. On wasm those SDL calls
                    // are stubbed to return IntPtr.Zero (see
                    // source/webclient/native-shims/sdl3_stubs.c:665), so the old
                    // `surface != Zero` gate swallowed the hotspot
                    // assignment entirely and `_cursorOffset` stayed
                    // all-zero. The GameCursor sprite then drew with
                    // its top-left pinned to Mouse.Position, offsetting
                    // every click / target by the hotspot amount
                    // (user-observed "apunta donde no debe"). Assign
                    // unconditionally; only the native SDL cursor
                    // creation needs the surface.
                    if (hotX != 0 || hotY != 0)
                    {
        ...
        public void Draw(UltimaBatcher2D sb)
        {
            ...
            // cursors rendered inside the GL canvas.
#if BROWSER_WASM
            if (true)
#else
