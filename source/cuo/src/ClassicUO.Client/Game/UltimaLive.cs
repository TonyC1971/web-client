    ...
    public sealed class UltimaLive
    {
        ...
        public class ULMapLoader : MapLoader
        {
            ...
            public void CheckForShardMapFile(int mapId)
            {
                ...
                // Adventures). Ported from the TazUO fix (operator-authorized source/cuo
                // edit 2026-06-03, scoped to the UltimaLive custom-shard fixes). The shadow
                // map/statics/staidx files this loader reads MUST be seeded from the REAL
                // files at /uo/. The old code asked GetMapFile(mapId) whether a loaded map
                // existed, but that reads _currentMapFiles, which is still all-null here:
                // Enable() calls CheckForShardMapFile at the 0x3F/0x01 case BEFORE this
                // loader's Load() runs, so the lookup always returned null -> it wrote a
                // BLANK map+staidx+statics, and once those blanks existed the statics/staidx
                // copies below were skipped too, leaving the entire facet BLACK.
                //
                // A facet is "real" only when the full triplet (map+staidx+statics) is
                // present on disk (standard .mul facets, e.g. adventures downloads
                // map0-5/statics0-5/staidx0-5.mul + the custom map33-36 set). Seed all three
                // from /uo/ and the terrain + statics render immediately; the server then
                // streams its live edits on top. Anything without a real base (a server
                // custom facet with no base, or a .uop-only map) falls back to a blank facet
                // the server streams into. Each CopyFile is guarded by File.Exists on the
                // source (CopyFile does not guard the source) and destination.
