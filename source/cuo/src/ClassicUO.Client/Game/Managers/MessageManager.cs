    ...
    internal sealed class MessageManager
    {
        ...
        )
        {
            ...
            (
                ...
                false,
                false,
                // Bug O29: non-OBJECT overheads (chat / yell / guild /
                // spell / party) were created with saveHitmap=false,
                // which made PixelCheck fall back to bounding-box hit
                // → clicking in empty space inside a chat bubble
                // stole clicks from the tile / item behind it. Use a
                // real hit-map for ALL overhead text so the click
                // lands on the text only when it actually touches
                // glyph pixels.
                saveHitmap: true
            );
