    ...
    internal sealed class HealthLinesManager
    {
        ...
        public void Draw(UltimaBatcher2D batcher, float layerDepth)
        {
            ...
            foreach (Mobile mobile in _world.Mobiles.Values)
            {
                ...
                // Bug O20: skip only when FULLY off-screen; previously
                // the bar vanished as soon as any edge crossed the
                // viewport boundary. Let partial overlap render (the
                // batcher scissor handles clipping).
                if (p.X + BAR_WIDTH <= 0 || p.X >= camera.Bounds.Width)
                {
                    continue;
                }

                if (p.Y + BAR_HEIGHT <= 0 || p.Y >= camera.Bounds.Height)
                {
                    continue;
                }
