...
namespace ClassicUO.Game.Managers
{
    internal static class UIManager
    {
        ...
        public static void Draw(UltimaBatcher2D batcher)
        {
            ...
            batcher.Begin();
#if BROWSER_WASM
            batcher.SetStencil(DepthStencilState.None);
#else
...
#if BROWSER_WASM
            // wasm Z-order fix (user report 2026-04-22: Skills/Journal
            // text bleeds over backpack when the backpack overlaps).
            //
            // Root cause: on wasm DepthStencilState is None (no depth
            // buffer on the direct-to-backbuffer path — see
            // GameController.Draw #if BROWSER_WASM), so per-vertex
            // layerDepth is ignored at rasterize time and pure draw-
            // order decides the final Z. The old code populated one
            // shared RenderLists for ALL gumps, then drew three fixed
            // buckets (transparent -> atlas -> noAtlas-text). Gumps
            // that wrap their children in ResizePic/HtmlControl keep
            // their subtree in a nested render list and draw as one
            // lambda — so the backpack (ContainerGump + ResizePic)
            // emits ONE lambda in the atlas bucket. But Skills +
            // Journal use ExpandableScroll (plain Control subclass)
            // which bubbles each Label up to the main noAtlas bucket.
            // Result: backpack's single lambda fires first, then the
            // dozen Skills labels fire, drawing ON TOP of the backpack
            // regardless of z-stack order.
            //
            // Fix: populate + draw per-gump. Each top-level gump gets
            // the buckets to itself, draws completely, then the next
            // one draws. The between-gump flush is implicit (texture-
            // switch or explicit) and cheap vs the correctness win.
            for (LinkedListNode<Gump> last = Gumps.Last; last != null; last = last.Previous)
            {
