...
namespace ClassicUO.Game.Managers
{
    internal sealed class AudioManager
    {
        ...
        // WASM now plays via Web Audio API directly (see Sound.cs
        // BROWSER_WASM branch + source/webclient/native-shims/SDL3.c wasm_play_*
        // shims + main.js wireWasmAudio). FAudio is still not
        // linked, but the Web Audio bridge covers UOSound (raw PCM)
        // + UOMusic (MP3) end-to-end so the rest of this manager
        // works unchanged on both targets.
        ...
        {
#if BROWSER_WASM
            // On wasm we do NOT instantiate DynamicSoundEffectInstance
            // (would throw DllNotFoundException because FAudio isn't
            // linked). Sound.Play's #if BROWSER_WASM branch routes
            // through Web Audio via wasm_play_pcm / wasm_play_music.
            // No sanity probe needed — the JS side silently no-ops
            // if AudioContext isn't available yet.
            Log.Trace("Audio enabled on BROWSER_WASM via Web Audio bridge");
#else
            try
            {
...
#if !BROWSER_WASM
            // SoundEffect.MasterVolume is FAudio-backed — would throw
            // DllNotFoundException on wasm. Web Audio handles the
            // focus-loss mute via the `visibilitychange` path in
            // main.js (AudioContext auto-pauses when the tab hides).
