...
namespace ClassicUO.Game.Scenes
{
    internal partial class GameScene
    {
        ...
        private bool OnLeftMouseDoubleClick()
        {
            ...
            if (!UIManager.IsMouseOverWorld)
            {
                // v0.4.14 dclick-diag: DC arrived but cursor is over a gump,
                // not the world. The "horse mount" path can't fire here.
                Console.WriteLine($"[dclick-diag] OnLeftMouseDoubleClick: !IsMouseOverWorld (cursor over gump) delayed_click={_world.DelayedObjectClickManager.IsEnabled}");
            ...
            // v0.4.14 dclick-diag: SelectedObject is the per-frame hover cache
            // populated during the world-render pass. If null/wrong type when
            // the user thinks they double-clicked a horse, the second click
            // landed on a different sub-tile (or static under the horse).
            string _objKind = obj switch
            {
                Item => "Item",
            ...
            Console.WriteLine($"[dclick-diag] OnLeftMouseDoubleClick: SelectedObject={_objKind}");
