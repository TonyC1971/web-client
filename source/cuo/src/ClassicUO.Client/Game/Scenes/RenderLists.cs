...
namespace ClassicUO.Game.Scenes
{
    ...
    internal class RenderLists
    {
        ...
        // v0.4.20.2: read-only views over the per-category lists. Used by
        // GameScene's idle-skip cache hit path to re-run CheckMouseSelection
        // without redoing the heavy chunk/mesh build. The cache (v0.3.47)
        // skips FillGameObjectList entirely when nothing relevant changed,
        // including mouse-still-over-world frames; without this pass,
        // SelectedObject (reset to null at the start of every DrawWorld) was
        // never repopulated on cache-hit frames, so clicks fired during a
        // static cursor returned target=null.
        public IReadOnlyList<GameObject> Tiles => _tiles;
        public IReadOnlyList<GameObject> StretchedTiles => _stretchedTiles;
        public IReadOnlyList<GameObject> Statics => _statics;
        public IReadOnlyList<GameObject> Animations => _animations;
        public IReadOnlyList<GameObject> Transparents => _transparentObjects;
