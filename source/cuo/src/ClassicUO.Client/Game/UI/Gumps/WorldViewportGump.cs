...
namespace ClassicUO.Game.UI.Gumps
{
    internal class WorldViewportGump : Gump
    {
        ...
        public Point ResizeGameWindow(Point newSize)
        {
            ...
            // Persist the new size to the profile immediately. Desktop
            // only syncs `profile.GameWindowSize` in `GameScene.Unload`
            // (the logout path), so on wasm — where tab-close rarely
            // fires Unload — the profile NEVER learns about a
            // drag-resize, and every re-login the viewport gets
            // force-snapped back to the wasm default (70 % of
            // browser canvas). Writing here lets WasmViewport.Resize
            // Game's "respect-saved-size" policy actually see the
            // user's choice on the next session. User report
            // 2026-04-22: "el resize al 70% cada vez que hago login
            // sigue persistiendo. No guarda el que yo había hecho
            // antes." — same gap shows up on desktop too if the
            // player crashes before logout, just less visibly.
            var profile = ClassicUO.Configuration.ProfileManager.CurrentProfile;
            if (profile != null)
            {
                profile.GameWindowSize = newSize;
            }
    ...
    internal class BorderControl : Control
    {
        ...
        public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
        {
            ...
            // browser-wasm: atlas path drops small sprites (O8).
            #if BROWSER_WASM
