...
namespace ClassicUO.Game.UI.Gumps
{
    ...
    internal class WorldMapGump : ResizableGump
    {
        ...
        public void SaveSettings()
        {
            ...
            // Bug O2: WorldMapGump mutates persistent profile fields
            // from ~20 call sites (every option-toggle ContextMenuItem
            // above) but never writes the profile to disk. Rely on
            // GameController's next auto-save tick instead of waiting
            // up to 5 s.
            GameController.RequestImmediateAutoSave();
...
#if BROWSER_WASM
            // v0.5.6: await the async server-PNG prefetch BEFORE LoadMap so a
            // cache hit lets LoadMap skip the 30-90 s in-browser rebuild. The
            // prefetch is async end-to-end — never blocks a thread on fetch.
            if (_loadingTask is { Status: TaskStatus.Running })
                _loadingTask = _loadingTask.ContinueWith(async _ =>
                {
                    await PrefetchMapPngAsync(index);
                    LoadMap(index);
                }).Unwrap();
            else
...
#if BROWSER_WASM
        // v0.5.6: async pre-fetch of the server-baked WorldMap PNG. Runs on
        // the loading task BEFORE LoadMap; on a 200 it writes the PNG into
        // _mapsCachePath so LoadMap's File.Exists check skips the 30-90 s
        // in-browser rebuild entirely.
        //
        // This MUST stay async. The previous version (v0.4.99) lived inside
        // LoadMap and blocked synchronously on the browser fetch via
        // `http.GetAsync(url).GetAwaiter().GetResult()`. A synchronous block
        // on a fetch traps the WASM runtime (uncatchable abort, not a C#
        // exception) — it crashed the client the moment CUO_SHARD_BASE was
        // non-empty (v0.5.5). `await` marshals the fetch to the main thread
        // and resumes the continuation; no thread is ever blocked.
        private async Task PrefetchMapPngAsync(int mapIndex)
        {
            try
            {
                if (mapIndex < 0 || mapIndex > MapLoader.MAPS_COUNT || !World.InGame)
                    return;

                string shardBase = Environment.GetEnvironmentVariable("CUO_SHARD_BASE");
                // v0.5.7: .NET's HttpClient rejects a relative request URI
                // with no BaseAddress (net_http_client_invalid_requesturi),
                // so the URL must be absolute. CUO_HTTP_ORIGIN carries the
                // page origin (e.g. https://uonexus.com) from main.js.
                ...
                // Same fingerprint LoadMap computes below — the .mul byte
                // sizes — so a hit here is the exact file LoadMap looks for.
            ...
            {
                // Any failure → LoadMap falls back to the in-browser rebuild.
        ...
        private unsafe void LoadMap(int mapIndex)
        {
            ...
            try
            {
                ...
                {
#if BROWSER_WASM
                    // v0.4.70: WorldMap fingerprint refactor. v0.4.69 used
                    // `new FileInfo(...).Length` which throws silently in
                    // Emscripten FS — the Task.Run wrapper swallows the
                    // exception, gump stays blank forever, no error logged.
                    // FileReader (returned by GetMapFile) already exposes
                    // `Length` directly from its underlying stream, so the
                    // file-size fingerprint costs zero extra IO + zero
                    // risk of WASM FS quirks. Skips the 146 MB MD5 hash
                    // (20-60 s of Mono interpreted hashing) that v0.4.69
                    // was meant to replace.
                    long mapLen = mapFile.Length;
                    long staticLen = staticFile.Length;
                    fileMapPath = Path.Combine(_mapsCachePath, $"map{mapIndex}_sz_{mapLen}_{staticLen}.png");
#else
                ...
                // v0.5.6: the server-baked PNG prefetch used to live here and
                // blocked synchronously on HttpClient — which traps the WASM
                // runtime. It now runs as the async PrefetchMapPngAsync,
                // awaited on the loading task BEFORE LoadMap (see ChangeMap),
                // so by the time we reach this point fileMapPath already
                // exists on a cache hit and the rebuild below is skipped.
        ...
        private void DrawAll(UltimaBatcher2D batcher, Rectangle srcRect, int gX, int gY, int halfWidth, int halfHeight, float layerDepth)
        {
            ...
            // UOAM live position-sharing peers (pushed by the rail /uoam client) —
            // drawn on the map next to party/guild members via DrawWMEntity.
            var uoamPeers = ClassicUO.Game.RailBridgeApi.UoamPeers;
            for (int ui = 0; ui < uoamPeers.Count; ui++)
            {
                DrawWMEntity(batcher, uoamPeers[ui], gX, gY, halfWidth, halfHeight, Zoom, layerDepth);
            }
