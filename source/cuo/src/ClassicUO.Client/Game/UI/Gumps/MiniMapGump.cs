...
namespace ClassicUO.Game.UI.Gumps
{
    internal class MiniMapGump : Gump
    {
        ...
        public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
        {
            ...
            float dotsLayerDepth = layerDepth + 0.001f;

            // Bug O31: map sprite + mobile dots used to share a single
            // AddGumpNoAtlas closure. Splitting into two closures lets
            // the batcher flush the atlas-sourced map texture
            // independently from the SolidColorTextureCache dot
            // textures. Dots are offset by a tiny layerDepth so they
            // always draw ON TOP of the map even in pure-linear order
            // (backup for the wasm per-gump path).
                ...
                renderLists.AddGumpNoAtlas(
                    batcher =>
                    {
                        ...
                        foreach (Mobile mob in World.Mobiles.Values)
                        {
                            ...
                            var dotHue = ShaderHueTranslator.GetHueVector(
                        ...
                        //DRAW PLAYER DOT
                        var playerHue = ShaderHueTranslator.GetHueVector(0);
