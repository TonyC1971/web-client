...
namespace ClassicUO.Game.UI.Gumps
{
    internal class NameOverheadGump : Gump
    {
        ...
        public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
        {
            ...
            // Bug O20: the old condition "fully or partially off-screen
            // -> don't render" hid names / HP labels for mobiles along
            // the viewport edges on smaller canvases. Only skip when
            // the label is FULLY off-screen; partial overlap should
            // render the visible portion (the batcher scissor handles
            // clipping naturally).
            if (x + Width <= camera.Bounds.X || x >= camera.Bounds.Right)
            {
                return false;
            }
            if (y + Height <= camera.Bounds.Y || y >= camera.Bounds.Bottom)
            {
                return false;
            }
