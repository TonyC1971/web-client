...
namespace ClassicUO.Game.UI.Gumps
{
    internal class OptionsGump : Gump
    {
        ...
        private HSliderBar _spriteSmoothingLevel;
        private Combobox _spriteSmoothingMode;
        ...
        public OptionsGump(World world) : base(world, 0, 0)
        {
            ...
            // Webclient-only page (operator 2026-07-10: "un submenu aparte para
            // el sprite smoothing"). Literal string — no upstream ResGumps entry.
            Add
            (
                new NiceButton
                (
        ...
        // Webclient-only page 13 (operator 2026-07-10: "un submenu aparte para el
        // sprite smoothing... con qué tipo de smooth poner"). One mode selector +
        // a strength slider for the texel modes. Hot-applied on OK/Apply — the
        // scene reads the profile per frame, no restart. Literal strings: product
        // UI is English-only and these knobs have no upstream ResGumps entry.
        private void BuildSpriteSmoothing()
        {
            const int PAGE = 13;
            ...
            // Performance warning FIRST, above the selector (operator 2026-07-28: players
            // on older GPUs were reporting heavy lag with this on). It is off by default
            // again and this is what they read before turning it back on.
            SettingsSection warnSection = AddSettingsSection(box, "Performance");
            warnSection.Add(AddLabel(null, "WARNING: smoothing costs GPU time. On older or", startX, startY));
            warnSection.Add(AddLabel(null, "integrated graphics it can drop the frame rate hard,", startX, startY));
            warnSection.Add(AddLabel(null, "especially the xBR / xBRZ upscale modes.", startX, startY));
            warnSection.Add(AddLabel(null, "If the game stutters, set this back to Off.", startX, startY));

            SettingsSection modeSection = AddSettingsSection(box, "Smoothing mode");
            modeSection.Y = warnSection.Bounds.Bottom + 40;
            modeSection.Add(AddLabel(null, "How sprite edges are treated:", startX, startY));
            modeSection.Add
            (
                _spriteSmoothingMode = AddCombobox
                (
                    null,
                    new[]
                    {
                        "Off - sharp pixels (classic)",
                        "Silhouette AA - soften edges only",
                        "Full AA - edges and interiors",
                        "xBR upscale - GPU pattern reconstruction",
                        "xBRZ upscale - GPU, highest quality"
                    },
                    ResolveSmoothingModeIndex(),
                    startX,
                    startY,
                    260
                )
            ...
            SettingsSection strengthSection = AddSettingsSection(box, "Strength");
            // Sections do NOT auto-stack inside the DataBox — every page chains
            // them manually (see BuildVideo: section2.Y = section.Bounds.Bottom
            // + 40). Missing this left all three drawing at Y=0 on top of each
            // other (operator screenshot 2026-07-10).
            strengthSection.Y = modeSection.Bounds.Bottom + 40;
            strengthSection.Add(AddLabel(null, "AA strength (Silhouette / Full modes)", startX, startY));
            // On its own line, NOT AddRight. The ScrollArea is WIDTH-210 = 490px wide and
            // has to fit this label, the 200px slider, the value the slider draws to its
            // right, AND the scrollbar. AddRight put the slider after a label this long
            // and it ran under the scrollbar (operator screenshot 2026-07-28). The
            // neighbouring TerrainShadowsLevel gets away with AddRight because its label
            // is a fraction of the width.
            strengthSection.Add(_spriteSmoothingLevel = AddHSlider(null, 0, 100, _currentProfile.SpriteSmoothingLevel, startX, startY, 200));

            SettingsSection notesSection = AddSettingsSection(box, "Notes");
            notesSection.Y = strengthSection.Bounds.Bottom + 40;
            notesSection.Add(AddLabel(null, "AA modes blend at the texel grid - nearly free.", startX, startY));
            notesSection.Add(AddLabel(null, "xBR / xBRZ redraw the scene through a GPU", startX, startY));
            notesSection.Add(AddLabel(null, "reconstruction filter: best quality, MUCH higher GPU cost.", startX, startY));
            notesSection.Add(AddLabel(null, "All modes apply instantly - no restart needed.", startX, startY));

            Add(rightArea, PAGE);
        }

        // Profile → combobox index. SpriteSmoothingMode -1 is the legacy
        // sentinel (pre-selector profiles): derive from Level+Full exactly
        // like GameScene.ResolveSpriteSmoothingMode does.
        private int ResolveSmoothingModeIndex()
        {
            int mode = _currentProfile.SpriteSmoothingMode;
            if (mode < 0)
            {
                mode = _currentProfile.SpriteSmoothingLevel <= 0 ? 0 : (_currentProfile.SpriteSmoothingFull ? 2 : 1);
            }
            return Math.Clamp(mode, 0, 4);
        }
        ...
        private void SetDefault()
        {
            switch (ActivePage)
            {
                ...
                case 3: // video
                    ...
                    _spriteSmoothingLevel.Value = 0;
                    _spriteSmoothingMode.SelectedIndex = 0;
        ...
        private void Apply()
        {
            ...
            {
                int smoothingMode = _spriteSmoothingMode.SelectedIndex;
                int smoothingLevel = _spriteSmoothingLevel.Value;
                if ((smoothingMode == 1 || smoothingMode == 2) && smoothingLevel <= 0)
                {
                    // Picking an AA mode with strength 0 would be a silent
                    // no-op — give it a sensible default and reflect it in
                    // the slider so the UI tells the truth.
                    smoothingLevel = 35;
                    _spriteSmoothingLevel.Value = 35;
                }
                _currentProfile.SpriteSmoothingMode = smoothingMode;
                _currentProfile.SpriteSmoothingLevel = smoothingLevel;
                // Legacy pair kept in sync for the v1 rail bridge readers.
                _currentProfile.SpriteSmoothingFull = smoothingMode == 2;
            }
            ...
            // Bug O2: flush IDBFS immediately after the profile
            // write so the user's option changes persist even if
            // they close the tab before the 5 s auto-save tick.
        ...
        private class InputField : Control
        {
            ...
            // Bug O33: allocating a fresh `new RenderLists()` inside
            // the per-frame closure spammed the GC. Cache one
            // instance per InputField and reuse it; Clear before
            // each use avoids the leak.
            private readonly RenderLists _cachedRenderLists = new();
            ...
            public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
            {
                ...
                renderLists.AddGumpNoAtlas(batcher =>
                {
                    ...
                    {
                        _cachedRenderLists.Clear();
                        base.AddToRenderLists(_cachedRenderLists, x, y, ref layerDepth);

                        _cachedRenderLists.DrawRenderLists(batcher, sbyte.MaxValue);
                        batcher.ClipEnd();
                    }
                    return true;
                });
