...
namespace ClassicUO.Game.UI.Gumps
{
    internal sealed class TradingGump : TextContainerGump
    {
        ...
        // Flag flipped by the server-initiated close path (type=1 in
        // PacketHandlers 0x6F). Dispose then skips the CancelTrade
        // packet that would otherwise be a redundant round-trip with
        // a server that already finalised the trade. Bug O53.
        public bool SuppressCancelOnDispose { get; set; }

        public override void Dispose()
        {
            base.Dispose();
            if (!SuppressCancelOnDispose)
            {
