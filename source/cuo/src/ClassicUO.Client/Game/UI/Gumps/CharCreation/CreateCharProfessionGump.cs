...
namespace ClassicUO.Game.UI.Gumps.CharCreation
{
    ...
    internal class ProfessionInfoGump : Control
    {
        ...
        public ProfessionInfoGump(ProfessionInfo info)
        {
            ...
            // Legacy LBR Prof.txt format keeps Localization=0 / Description=0
            // when only `Name`/`Desc <int>` keys are present (no `nameid` /
            // `descid`). Without this fallback the label renders "MegaCliloc:
            // missing" and the tooltip shows the same. ProfessionLoader
            // already parses `Name` into info.Name (literal trade name like
            // "Warrior"), so use that when the cliloc lookup would fail.
            //
            // v0.3.20: also defend against info.Name being null/empty (a
            // category-type entry that ships only `nameid` and the cliloc
            // lookup also fails). Falling back to TrueName (the section
            // header, e.g. "Warrior" / "Magician" / "Adventurer") keeps
            // the label readable instead of rendering an empty button.
            string profLabel = localization.GetString(info.Localization, fallback: info.Name);
            if (string.IsNullOrWhiteSpace(profLabel)) profLabel = info.TrueName ?? string.Empty;
            string profTooltip = localization.GetString(info.Description, fallback: string.Empty);
            background.SetTooltip(profTooltip, 250);

            Add(background);

            Add
            (
                new Label(profLabel, true, 0x00, font: 1)
                {
                    X = 7,
                    Y = 8
                }
