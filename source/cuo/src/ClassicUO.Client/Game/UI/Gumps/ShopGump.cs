...
namespace ClassicUO.Game.UI.Gumps
{
    internal class ShopGump : Gump
    {
        ...
        private class ShopItem : Control
        {
            ...
            public int Amount
            {
                // Bug O56: int.Parse throws FormatException on blank /
                // non-numeric text (possible during a dispose + Update
                // race). Guard with TryParse so a stale label never
                // kills the render loop.
                get => int.TryParse(_amountLabel.Text, out var n) ? n : 0;
