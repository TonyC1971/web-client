    ...
    internal class PaperDollGump : TextContainerGump
    {
        ...
        protected override void OnMouseUp(int x, int y, MouseButtonType button)
        {
            if (button == MouseButtonType.Left && World.InGame)
            {
                ...
                // Bug O54: if the target mobile just went out of
                // sight / logged off / got removed, `container` is
                // null and the FindItemByLayer call below NREs. We
                // still let the drop fall through for cases that
                // don't dereference container (e.g. drop onto a
                // container item at cursor).
                if (container == null && LocalSerial != World.Player.Serial)
                {
                    return;
                }
            ...
            private class ItemGumpFixed : ItemGump
            {
                ...
                public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
                {
                    ...
                    if (texture != null)
                    {
                        ...
                        // Same atlas-path hue-drop trap the LoginGump
                        // checkbox hit (commit 77c86bae6). Under
                        // browser-wasm, AddGumpWithAtlas renders the
                        // sprite but drops the hueVector, so items
                        // whose base tint comes from a hue byte
                        // (hued tunics, dyed robes) render in the
                        // sprite's raw greyscale.
                        // Routing through AddGumpNoAtlas restores the
                        // hue application.
#if BROWSER_WASM
