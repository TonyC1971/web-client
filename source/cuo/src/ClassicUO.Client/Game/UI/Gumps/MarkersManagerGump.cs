...
namespace ClassicUO.Game.UI.Gumps
{
    internal sealed class MarkersManagerGump : Gump
    {
        ...
        internal MarkersManagerGump(World world) : base(world, 0, 0)
        {
            ...
            // maxwidth 0 = auto-size: 40px forced "Remove" to wrap to a second
            // line ("Remov/e" — player report 2026-07-18, operator-sanctioned
            // cuo edit). GoTo nudged +5 so the full-width header can't touch it.
            Add(new Label(ResGumps.Remove, true, HUE_FONT, 0, 255, FontStyle.BlackBorder) { X = 500, Y = initY });

            Add(new Label(ResGumps.MarkerGoTo, true, HUE_FONT, 0, 255, FontStyle.BlackBorder) { X = 555, Y = initY });
