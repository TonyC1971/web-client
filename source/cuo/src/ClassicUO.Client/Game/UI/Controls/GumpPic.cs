...
namespace ClassicUO.Game.UI.Controls
{
    ...
    internal class GumpPic : GumpPicBase
    {
        ...
        public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
        {
            ...
            if (gumpInfo.Texture != null)
            {
                ...
                // Same atlas-path trap the Checkbox (77c86bae6),
                // PaperDollGump item, and Button sprite hit on
                // browser-wasm: AddGumpWithAtlas drops small UI
                // sprites. GumpPic is the most reused UI control in
                // the client (journal headers, backgrounds, icons),
                // so this fix propagates to many downstream gumps.
#if BROWSER_WASM
    ...
    internal class GumpPicInPic : GumpPicBase
    {
        ...
        public override bool AddToRenderLists(RenderLists renderLists, int x, int y, ref float layerDepthRef)
        {
            ...
            if (texture != null)
            {
                // Pic-in-pic variant. Same BROWSER_WASM gate as above.
#if BROWSER_WASM
                renderLists.AddGumpNoAtlas(
#else
                renderLists.AddGumpWithAtlas(
