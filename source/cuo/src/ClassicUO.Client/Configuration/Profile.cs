...
namespace ClassicUO.Configuration
{
    ...
    internal sealed class Profile
    {
        ...
        public bool UseAlternativeLights { get; set; } = true; // v0.5.12: ON by default for new profiles
        ...
        // slider-driven, never breaks sprites. Level 0..100 (0 = off, the
        // legacy bit-exact render); Full switches from silhouette-only AA
        // (alpha edges) to per-tap hue-resolved interior smoothing.
        public int SpriteSmoothingLevel { get; set; } = 0;

        public bool SpriteSmoothingFull { get; set; } = false;

        // 5-way selector (task #163): -1 = legacy sentinel (derive from the
        // level+full pair above so pre-selector profiles keep their choice),
        // 0=off 1=silhouette 2=full 3=xBR 4=xBRZ.
        public int SpriteSmoothingMode { get; set; } = -1;
        ...
        public bool SmoothingResetV2 { get; set; } = false;
        ...
        // v0.4.88: default changed from true to false. Operator request to
        // ship first-time profiles with this off and to force off for Guest
        // sessions. Used by HealthLinesManager + MacroManager to draw a
        // distinct healthbar style on the target serial set via the
        // NewTarget* macro path. Players who want it can enable it from
        // Options > General.
