...
#if BROWSER_WASM
            // Bug O1: ExecutablePath resolves to `/` on wasm (no
            // chdir; emscripten default cwd). IDBFS is mounted at
            // `/Data` — anything written OUTSIDE that tree never
            // persists. Route settings.json into /Data so SaveAccount,
            // LastServerName, WindowPosition etc. actually survive
            // across reloads.
            return Path.Combine(CUOEnviroment.ExecutablePath, "Data", SETTINGS_FILENAME);
#else
