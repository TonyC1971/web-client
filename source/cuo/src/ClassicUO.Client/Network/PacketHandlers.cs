...
namespace ClassicUO.Network
{
    sealed class PacketHandlers
    {
        ...
        // v0.4.13: per-serial dedup cache for the "No container (X) found"
        // warning at AddItemToContainer. Server bursts produce ~70 lines
        // per second for the same parent serial post-teleport; without
        // dedup the log is unreadable. See packet handler at the warning
        // site for the eviction logic.
        private static readonly System.Collections.Generic.Dictionary<uint, long> _noContainerWarnRecent
...
#if BROWSER_WASM
            // v0.4.64: time-budget packet throttle. Replaces v0.4.63's
            // fixed count cap (40 packets/frame) with a 25 ms wall-clock
            // cap. Adaptive to per-packet cost variance: cheap packets
            // (ack/ping/mobile-status) flow through, expensive packets
            // (chunk-data, mass-mobile-spawn) trigger the bail sooner.
            //
            // The CircularBuffer was already designed for partial-data
            // continuation (line 146 `stream.Length<packetlength:break`
            // already holds bytes across frames), so deferring packets
            // is just a soft cap on top of the existing mechanism.
            //
            // Cap chosen: 25 ms. Aligns roughly with v0.4.63's empirical
            // good cap (40 packets × ~0.6 ms avg ≈ 24 ms) but bails
            // earlier on heavy-packet bursts where 40 × 3 ms would
            // produce a 120 ms slice. So worst-case per-frame packet
            // work is bounded to ~25-30 ms regardless of packet mix.
            //
            // Order preservation unchanged: protocol-critical packets
            // (0x1B position) always arrive first in the burst, fit
            // inside the first frame's 25 ms slice. Static / mobile
            // fill-in spreads.
            // Checkpoint B (default on master) — peak-protection profile.
            // 20ms wall-clock cap on the packet pump per frame. Sibling
            // release v0.4.65-A uses long.MaxValue here for the
            // max-smoothness profile. See release-highlights-v0.4.65-A.md
            // and release-highlights-v0.4.65-B.md for trade-off detail
            // and 50-iter validated stats. To swap to A locally: set to
            // long.MaxValue, build, deploy. Master default is B because
            // it eliminates >500ms catastrophes which matched the
            // operator priority "no permitir esos picos".
            const long MAX_MS_PER_FRAME = 20;
            var __pumpSw = System.Diagnostics.Stopwatch.StartNew();
            return ParsePackets(socket, world, _buffer, true, __pumpSw, MAX_MS_PER_FRAME) +
                   ParsePackets(socket, world, _pluginsBuffer, false, __pumpSw, MAX_MS_PER_FRAME);
#else
...
#if BROWSER_WASM
        private int ParsePackets(NetClient socket, World world, CircularBuffer stream, bool allowPlugins, System.Diagnostics.Stopwatch sw, long maxMs)
        {
            var packetsCount = 0;
            ...
            lock (stream)
            {
                ...
                while (stream.Length > 0 && sw.ElapsedMilliseconds < maxMs)
                {
#else
        private int ParsePackets(NetClient socket, World world, CircularBuffer stream, bool allowPlugins)
        {
            ...
            lock (stream)
            {
                ...
                while (stream.Length > 0)
                {
                    ...
                    // Guard: a zero-length entry in PacketsTable would
                    // infinite-loop (Dequeue(0) never shrinks the stream).
                    // This happens on wasm when Huffman trailing bytes
                    // produce a fake packet that misaligns the stream
                    // until it lands on an ID whose table entry is 0.
                    if (packetlength <= 0)
                    {
                        Log.Warn($"zero-length packet ID: {packetID:X2} -- dropping 1 byte");
        ...
        private static void SecureTrading(World world, ref StackDataReader p)
        {
            ...
            {
                // Server already closed the trade; skip the redundant
                // CancelTrade round-trip that Dispose would otherwise
                // send (bug O53).
                var tg = UIManager.GetTradingGump(serial);
                if (tg != null)
                {
                    tg.SuppressCancelOnDispose = true;
                    tg.Dispose();
                }
            }
            else if (type == 2)
        ...
        private static void EnterWorld(World world, ref StackDataReader p)
        {
            uint serial = p.ReadUInt32BE();
#if BROWSER_WASM
            // Bug 1 trace: on a second login after logout, LoginComplete
            // fired with player=null, meaning this EnterWorld handler
            // either didn't run or ran and failed to set Player. Log
            // entry + exit so we can tell which.
            var _prof = ClassicUO.Configuration.ProfileManager.CurrentProfile == null ? "null" : "set";
            var _acct = ClassicUO.Game.Scenes.LoginScene.Account ?? "null";
            var _srv = world.ServerName ?? "null";
            var _lc = string.Empty;
            try
            {
                _lc = ClassicUO.Game.Managers.LastCharacterManager.GetLastCharacter(_acct, _srv) ?? string.Empty;
            }
            catch { _lc = "<throw>"; }
            ClassicUO.Utility.ZoneLoadDiag.Anchor();
            WasmTrace.W($"[zonediag] t=0ms EnterWorld enter serial=0x{serial:X8} profile={_prof} account={_acct} server={_srv} lastChar='{_lc}'");

            try
            {
                world.CreatePlayer(serial);
                WasmTrace.W($"[zonediag] t={ClassicUO.Utility.ZoneLoadDiag.Ms}ms after CreatePlayer player={(world.Player == null ? "null" : "set")}");
            }
            catch (System.Exception ex)
            {
                WasmTrace.W($"[cuo-trace] EnterWorld CreatePlayer THREW: {ex.GetType().Name}: {ex.Message}");
                throw;
            }
#else
            world.CreatePlayer(serial);
            ...
            world.RangeSize.Y = y;
#if BROWSER_WASM
            WasmTrace.W($"[zonediag] t={ClassicUO.Utility.ZoneLoadDiag.Ms}ms after SetInWorldTile({x},{y},{z}) map={world.MapIndex}");
#endif
...
#if BROWSER_WASM
            WasmTrace.W($"[zonediag] t={ClassicUO.Utility.ZoneLoadDiag.Ms}ms after Send_ClientVersion");
#endif

            GameActions.SingleClick(world, world.Player);
            NetClient.Socket.Send_SkillsRequest(world.Player.Serial);
#if BROWSER_WASM
            WasmTrace.W($"[zonediag] t={ClassicUO.Utility.ZoneLoadDiag.Ms}ms after SingleClick+SkillsRequest");
#endif
...
#if BROWSER_WASM
            WasmTrace.W($"[zonediag] t={ClassicUO.Utility.ZoneLoadDiag.Ms}ms EnterWorld exit (all initial sends issued)");
#endif
        }
        ...
        private static void OpenContainer(World world, ref StackDataReader p)
        {
            ...
            else if (graphic == 0x0030)
            {
                ...
                for (Layer layer = Layer.ShopBuyRestock; layer < Layer.ShopBuy + 1; layer++)
                {
                    ...
                    // Bug O51: FindItemByLayer returns null when the
                    // vendor has no stock on that layer (training
                    // vendors, partially-stocked shops on custom
                    // shards). Dereferencing it.Items NREs and kills
                    // the whole packet handler, freezing the client.
                    // Null-guard BEFORE reading .Items.
                    if (item == null)
                    {
                        continue;
                    }
                ...
                )
                {
                    if (!Client.Game.UO.GameCursor.ItemHold.UpdatedInWorld)
                    {
                        ...
                        )
                        {
                            ...
                            // Console.WriteLine("=== DENY === ADD TO CONTAINER"); // O55
                        ...
                        else
                        {
                            ...
                            if (container != null)
                            {
                                ...
                                {
                                    // Console.WriteLine("=== DENY === ADD TO PAPERDOLL"); // O55
                                ...
                                {
                                    // Console.WriteLine("=== DENY === SOMETHING WRONG"); // O55
                            ...
                            {
                                // Console.WriteLine("=== DENY === ADD TO TERRAIN"); // O55
        ...
        private static void DropItemAccepted(World world, ref StackDataReader p)
        {
            ...
            // Console.WriteLine("PACKET - ITEM DROP OK!"); // O55
        }
        ...
        private static void DeathScreen(World world, ref StackDataReader p)
        {
            ...
            if (action != 1)
            {
                // Telemetry (operator 2026-06-11): own death + probable killer.
                RailBridgeApi.NotePlayerDeath(world);
        ...
        {
#if BROWSER_WASM
            // Diagnostic for the user-reported 2026-04-24 hang at
            // "Entering Britannia" on logout + relogin with a different
            // account. If we ever see this line fire with Player==null
            // or Scene!=LoginScene, we know the 0x55 arrived but was
            // silently swallowed by the guard below; without this log
            // the hang looks like "the packet never arrived".
            var _scene = Client.Game.Scene?.GetType().Name ?? "null";
            WasmTrace.W($"[cuo-trace] LoginComplete fired player={(world.Player == null ? "null" : "set")} scene={_scene}");
#endif
            if (world.Player != null && Client.Game.Scene is LoginScene)
            {
...
#if BROWSER_WASM
                // FIX 2026-04-22: emit sniff line for main.js so it can
                // dispatch `cuo:gamescene-active` DOM event. Needed
                // because wireDynamicResolution's `entering-britannia`
                // handler fires BEFORE Scene actually transitions here;
                // at that point WasmViewport.ResizeGame early-returns
                // (scene is still LoginScene), so the canvas stays at
                // 640x480. Listening for this second signal lets JS
                // retry the resize once GameScene is really active.
                // Fires `cuo:gamescene-active` in main.js to trigger
                // the canvas resize that ResizeGame deferred during
                // LoginScene. Direct C# -> JS, no console sniff.
                ...
                // FIRST-LOGIN preset (operator 2026-07-10 "a los que se conecten por primera
                // vez, o sean guest, precárgales la configuración: paperdoll, inventory,
                // minimap, stats auto-abiertos"): a profile that has never saved a gump
                // layout has no gumps.xml yet — that is exactly the first session of a
                // fresh browser/account (guests always; Discord users only until their
                // synced profile lands, which restores THEIR layout instead). Probe the
                // file BEFORE ReadGumps so a same-session save can't mask the signal.
                ...
                if (freshProfile)
                {
                    try
                    {
                        ...
                        // Fixed layout (operator 2026-07-10): the preset gumps open
                        // asynchronously, so a deferred pass snaps each one to its slot
                        // (paperdoll right of the game view, backpack under it, status +
                        // skills + minimap under the game view) as they appear.
...
#if BROWSER_WASM
        // Burst-tracking counters for the in-Britain step-hitch diagnosis.
        // Hitches reproduce on busy areas only (e.g. Britain), disappear in
        // empty zones (e.g. Green Acres). Hypothesis is that a step triggers
        // a server cascade — many UpdateCharacter / UpdateObject packets
        // arriving in one frame — and the small-GC churn of processing all
        // of them sums to a visible stall. These prints let the bot grep
        // them and
        // confirm/falsify before we pick a fix direction.
        private static int _broadcastCount;
        private static long _lastBroadcastWindowTicks;
        private static int _broadcastInWindow;

        private static void NoteBroadcast(string kind)
        {
            _broadcastCount++;
            long now = System.DateTime.UtcNow.Ticks;
            // 50 ms windows. If more than ~5 packets arrive in one window
            // and the wasm tick is 60 fps (16.7 ms), most likely the next
            // Draw frame eats them as one batch — a hitch.
            if (now - _lastBroadcastWindowTicks > 500_000)
            {
                if (_broadcastInWindow >= 5)
                {
                    WasmTrace.W(
                        $"[pktburst] window-end count={_broadcastInWindow} totalSinceLogin={_broadcastCount} kind={kind}"
                    );
                }
                _lastBroadcastWindowTicks = now;
                _broadcastInWindow = 1;
            }
            else
            {
                _broadcastInWindow++;
        ...
        private static void DisplayDeath(World world, ref StackDataReader p)
        {
            ...
            // Telemetry (operator 2026-06-11): credit a kill BEFORE the corpse
            // serial mutation below renames the mobile (serial | 0x80000000).
            RailBridgeApi.NoteMobileDeath(world, serial);
        ...
        )
        {
            if (Client.Game.UO.GameCursor.ItemHold.Serial == serial)
            {
                ...
                {
                    // Console.WriteLine("ADD ITEM TO CONTAINER -- CLEAR HOLD"); // O55
                    Client.Game.UO.GameCursor.ItemHold.Clear();
                }
            ...
            if (container == null)
            {
                // v0.4.13: dedup the warning per-serial within a 1 s window.
                // Operator log 2026-05-07 captured ~70 identical lines for
                // a single container serial in one second after teleport
                // (server pushed updates for items the client hasn't yet
                // received the parent for). One log line per serial is
                // diagnostically equivalent and saves the spam.
                long _ncNowMs = System.Environment.TickCount64;
                if (!_noContainerWarnRecent.TryGetValue(containerSerial, out long _ncLastMs)
                    || _ncNowMs - _ncLastMs >= 1000)
                {
                    Log.Warn($"No container ({containerSerial}) found");
                    _noContainerWarnRecent[containerSerial] = _ncNowMs;
                    if (_noContainerWarnRecent.Count > 256)
                    {
                        long _ncCutoff = _ncNowMs - 5000;
                        var _ncStale = new System.Collections.Generic.List<uint>();
                        foreach (var kv in _noContainerWarnRecent)
                        {
                            if (kv.Value < _ncCutoff) _ncStale.Add(kv.Key);
                        }
                        foreach (var k in _ncStale) _noContainerWarnRecent.Remove(k);
                    }
                }
        ...
        )
        {
            ...
            // v0.4.67 (S01) — Item state dedup. When the server re-broadcasts
            // 0xF3 for an existing item with identical state (happens on
            // chunk-edge crosses, [Runmatch revisits, range-entry refreshes),
            // skip the full reassignment + CheckGraphicChange + SetInWorldTile
            // chain. SetInWorldTile is the heavy part — it does
            // RemoveFromTile() + Chunk.AddGameObject() sorted-insert per call.
            // Multi items (type==2) excluded because their internal
            // WantUpdateMulti latch must clear precisely.
            bool itemStateUnchanged = false;
            if (item != null)
            {
                if (type != 2 && !created && !item.IsMulti && !item.IsDestroyed)
                {
                    ushort newGraphic = (graphic != 0x2006)
                        ? (ushort)(graphic + graphic_inc)
                        : graphic;
                    ushort newAmount = count == 0 ? (ushort)1 : count;

                    // Inline FixHue logic (Entity.FixHue) so dedup comparison
                    // matches what the assignment path would produce.
                    ushort newHueFixed;
                    {
                        ushort fc = (ushort)(hue & 0x3FFF);
                        if (fc != 0)
                        {
                            if (fc >= 0x0BB8) fc = 1;
                            fc |= (ushort)(hue & 0xC000);
                            newHueFixed = fc;
                        }
                        else
                        {
                            newHueFixed = (ushort)(hue & 0x8000);
                        }
                    }
                    ...
                    itemStateUnchanged =
                        ...
                        && item.Hue == newHueFixed
                        && item.Flags == flagss
                        && item.Amount == newAmount
                        && item.Direction == direction
                        && item.LightID == (byte)direction
                        && (newGraphic != 0x2006 || item.Layer == (Layer)direction);
                }

                if (!itemStateUnchanged)
                {
                    if (graphic != 0x2006)
                    {
                        graphic += graphic_inc;
            ...
            else
            {
                ...
                if (item.OnGround)
                {
                    // v0.4.67 (S01): skip SetInWorldTile when dedup short-
                    // circuited everything — the item is already at its
                    // current tile, no work needed.
                    if (!itemStateUnchanged)
                    {
                        item.SetInWorldTile(item.X, item.Y, item.Z);
                    }
        ...
        )
        {
            if (serial == world.Player)
            {
                ...
                // v0.4.68: reset the player's walk-step state BEFORE
                // SetInWorldTile. Without this, `Player.Offset` retains the
                // residual interpolation displacement from a partially-
                // completed walk step (mid-stride when a `[tele` packet
                // arrived). GetViewPort's `winGameCenterX -= Player.Offset.X`
                // then shifts the rendered tile diamond off-axis relative to
                // the iterated tile range, leaving large black bands at the
                // canvas edges. Symptom: `[tele` of 5-15 tiles fills half the
                // screen with black squares that don't refresh (operator log
                // 2026-05-12 + screenshot). ClearSteps() resets Steps + sets
                // Offset = Vector3.Zero; safe to call here because the
                // teleport invalidates any pending walk steps anyway.
