...
namespace ClassicUO.Network.Encryption
{
    internal sealed unsafe class BlowfishEncryption
    {
        ...
        // Inverse of Encrypt for the same CFB-style stream. The keystream
        // generation (RawEncrypt every 8 bytes off the rolling _seed) is
        // identical, but the seed update has to feed the CIPHERTEXT (the
        // input here), not the plaintext output. Encrypt does:
        //     dst[i] = src[i] ^ keystream;  _seed[bp] = dst[i];   // = ciphertext
        // Decrypt mirrors that:
        //     plain  = cipher ^ keystream;  _seed[bp] = cipher;   // = input
        // Required for shards (Sphere with USECRYPT=1, BLOWFISH__2_0_3)
        // that encrypt s2c symmetrically; original retail UO 2.0.3 sent
        // s2c plaintext, which is why upstream CUO never had this path.
        public void Decrypt(Span<byte> src, Span<byte> dst, int size, ref int index_in, ref int index_out)
        {
            while (_stream_pos + size > Crypt_Constants.CRYPT_GAME_TABLE_TRIGGER)
            {
                int len_remaining = Crypt_Constants.CRYPT_GAME_TABLE_TRIGGER - _stream_pos;
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                byte ct = src[i];                               // capture before potential aliasing
                dst[i] = (byte) (ct ^ _seed[_block_pos]);
                _seed[_block_pos] = ct;                         // ← feedback uses input (ciphertext)
                _block_pos = (_block_pos + 1) % 8;
            }
