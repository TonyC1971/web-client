...
namespace ClassicUO.Renderer
{
    ...
    public sealed class FontGlyphAtlas : IDisposable
    {
        ...
        // Bug O30: colored-cache grows unbounded — Notoriety colours
        // + HTML tooltip `<basefont color=...>` segments allocate a
        // fresh atlas sprite per (glyph, color). Soft-cap the
        // colored cache at 4096 entries and clear when reached so
        // the 2048x2048 TextureAtlas can't exhaust after hours of
        // play. The actual atlas memory is recovered on the next
        // glyph cycle (new atlas pages are added as needed; old
        // ones stay backed by GPU VRAM until Dispose, but further
        // allocations work correctly).
        private const int COLORED_CACHE_MAX = 4096;
        ...
        public GlyphAtlasEntry GetColoredEntry(byte font, char character, bool isUnicode, bool hasBorder, bool isSolid, bool isItalic, uint color)
        {
            ...
            if (_coloredCache.Count >= COLORED_CACHE_MAX)
            {
                // Soft cap hit — clear colored entries to bound
                // memory. Next frame re-adds what's still on screen;
                // the _cache (uncoloured) survives so ordinary text
