...
// Sprite smoothing (operator 2026-07-10 "ligero antialiasing regulable
// por slider"). Texel-AA / sharp-bilinear done MANUALLY with 4 point
// taps at texel centres — the atlas has no HW-filter-safe padding
// story beyond the 1px transparent border TextureAtlas now packs, and
// point taps keep the off state bit-identical to the legacy render.
//   SmoothMode: 0 = off, 1 = silhouette (alpha-only AA — softens sprite
//               outlines, zero palette-artifact risk), 2 = full (per-tap
//               hue resolution + alpha-weighted colour blend).
//   SmoothWidth: 0..1 slider — scales the smoothstep transition band
//               around texel edges (fwidth-based, so it stays ~1 screen
//               pixel wide at any zoom/DPR).
//   DrawTexSize: bound texture dimensions, set by Batcher2D on texture
//               switch (tex2D-era HLSL has no textureSize()).
// Gated in-code to world sprites only: gumps (isGump), text modes,
// SHADOW/SPECTRAL/LIGHTS are excluded, so UI and overhead text stay
// crisp regardless of the slider.
