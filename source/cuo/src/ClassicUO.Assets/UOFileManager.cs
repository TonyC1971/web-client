...
namespace ClassicUO.Assets
{
    public sealed class UOFileManager : IDisposable
    {
        ...
        {
#if BROWSER_WASM
            // v0.3.19: every browser-side asset request goes through nginx
            // serving from a Linux fs. Both the writer side (operators
            // syncing .mul trees from Windows) and the reader side (this
            // method building URLs from mixed-case literal strings sprinkled
            // across the loaders — "Anim1.def", "MainMisc.uop", "Prof.txt",
            // "Citytext.enu", etc.) historically disagreed, so the deploy
            // stack maintained per-file Cap-first/lowercase symlinks via
            // gamefiles-case-fix.sh. That worked on bulk deploys but fell
            // over for any new file the operator added between case-fix
            // runs (recent Prof.txt + Stones2.mid one-offs). Since we own
            // the client we can flip the rule the other way: lowercase
            // every URL component here, before Path.Combine. Server side
            // standardises on lowercase, no symlinks, no case-fix step.
            // Note: BasePath isn't lowercased — operators still see
            // /gamefiles/server-2/... in dev tools as written, only the
            // file argument from the loader gets normalised.
            file = file.ToLowerInvariant();
            // The override-map lookup below is keyed lowercase already
            // (UOFilesOverrideMap.Load() normalises segments[0] to lower
            // before insert), and `file` is now lowercase too — pass it
            // straight through instead of re-lowercasing per call.
