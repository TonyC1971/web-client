...
namespace ClassicUO.Assets
{
    public sealed class ArtLoader : UOFileLoader
    {
        ...
        private static unsafe uint[] LoadArt(UOFile file, ref readonly UOFileIndex entry, out short width, out short height)
        {
            ...
            // `buf` is read, decoded in the `fixed` block below, then
            // dropped. Renting from the shared pool instead of `new`
            // keeps the wasm SGen nursery quiet during the sprite-load
            // burst that fires when the player walks into a new zone
            // (each fresh static graphic lands here exactly once).
            // Before: every LoadArt = one fresh `byte[entry.Length]`
            // + one fresh `uint[W*H]` allocation. Under sustained
            // cross-region walking, the nursery accumulated enough
            // pressure that `interp_mark_stack` eventually traps OOB
            // scanning roots (bug #2 in docs/KNOWN_BUGS.md).
            var buf = ArrayPool<byte>.Shared.Rent(entry.Length);
            try
            {
                var readSpan = buf.AsSpan(0, entry.Length);
                file.Read(readSpan);
