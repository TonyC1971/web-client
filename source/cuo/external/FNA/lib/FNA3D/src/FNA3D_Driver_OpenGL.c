...
static void OPENGL_SwapBuffers(
	...
	if (renderer->backbuffer->type == BACKBUFFER_TYPE_OPENGL)
	{
...
#ifdef WASM_DIAG_GL
		{
			...
			static int _wasm_swap_remaining = 4;
			if (_wasm_swap_remaining > 0)
			{
				unsigned int _err = glGetError();
				fprintf(stderr,
					"[FNA3D swap-OPENGL] src=(%d,%d %dx%d) dst=(%d,%d %dx%d) realFBO=%u fauxFBO=%u glGetError=0x%x\n",
					srcX, srcY, srcW, srcH, dstX, dstY, dstW, dstH,
					(unsigned)renderer->realBackbufferFBO,
					(unsigned)renderer->backbuffer->opengl.handle,
					_err);
				_wasm_swap_remaining--;
			}
		}
#endif
...
#ifdef WASM_DIAG_GL
		{
			static int _wasm_swap_null_remaining = 4;
			if (_wasm_swap_null_remaining > 0)
			{
				fprintf(stderr,
					"[FNA3D swap-NULL] backbuffer type=NULL, just SwapWindow\n");
				_wasm_swap_null_remaining--;
			}
		}
#endif
...
static void OPENGL_Clear(
...
#ifdef WASM_DIAG_GL
	{
		...
		static int _wasm_clear_remaining = 6;
		if (_wasm_clear_remaining > 0 && clearTarget)
		{
			int _fbo = -1;
			glGetIntegerv(0x8CA6 /*GL_DRAW_FRAMEBUFFER_BINDING*/, &_fbo);
			fprintf(stderr,
				"[FNA3D clear] color=(%.2f,%.2f,%.2f,%.2f) DRAW_FBO=%d realFBO=%u rtBound=%d preErr=0x%x\n",
				color->x, color->y, color->z, color->w,
				_fbo, (unsigned)renderer->realBackbufferFBO,
				(int)renderer->renderTargetBound, glGetError());
			_wasm_clear_remaining--;
		}
	}
#endif
	/* CLEAR! */
...
static void OPENGL_DrawIndexedPrimitives(
	...
	{
		extern unsigned int glGetError(void);
		static int _wasm_drawerr_remaining = 4;
		if (_wasm_drawerr_remaining > 0)
		{
			unsigned int _err = glGetError();
			fprintf(stderr,
				"[FNA3D draw] DrawRangeElements minIdx=%d numV=%d startIdx=%d primCount=%d baseV=%d -> glGetError=0x%x\n",
				(int)minVertexIndex, (int)numVertices, (int)startIndex,
				(int)primitiveCount, (int)baseVertex, _err);
			_wasm_drawerr_remaining--;
		}
	}
#endif
...
#ifdef WASM_DIAG_GL
				{
					static int _wasm_dump_remaining = 8;
					if (_wasm_dump_remaining > 0)
					{
						_wasm_dump_remaining--;
						fprintf(stderr,
							"[FNA3D bind] elem[%d] usage=%d index=%d offset=%d format=%d -> attribLoc=%d stride=%d\n",
							j, usage, index, (int)element->offset,
							(int)element->vertexElementFormat,
							(int)attribLoc, (int)vertexDeclaration->vertexStride);
					}
				}
#endif
				if (attribLoc == -1)
