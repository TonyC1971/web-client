    ...
    if (ctx->have_opengl_2)
    {
...
#ifdef WASM_DIAG_GL
        fprintf(stderr,
            "===== MOJOSHADER GLSL src type=%s len=%d ver=%d.%d sampler_count=%d output_count=%d attribute_count=%d =====\n%s\n===== END =====\n",
            (shader_type == 0x8B31 /*GL_VERTEX_SHADER*/) ? "VS" : "PS",
            (int)codelen, (int)pd->major_ver, (int)pd->minor_ver,
            (int)pd->sampler_count, (int)pd->output_count,
            (int)pd->attribute_count, pd->output);
#endif
        ...
        if (!ok)
        {
            ...
            ctx->glGetShaderInfoLog(shader, sizeof (error_buffer), &len,
                                 (GLchar *) error_buffer);
#ifdef WASM_DIAG_GL
            fprintf(stderr, "===== MOJOSHADER GLSL COMPILE FAIL =====\n%.*s\n===== END =====\n",
                                    ...
                                    MOJOSHADER_glShader *pshader)
{
    ...
    if (ctx->have_opengl_2)
    {
...
#ifdef WASM_DIAG_GL
        {
            GLsizei _len = 0;
            ctx->glGetProgramInfoLog(program, sizeof (error_buffer),
                                     &_len, (GLchar *) error_buffer);
            fprintf(stderr,
                "===== MOJOSHADER GLSL link status=%d log_len=%d prog=%u v=%u p=%u =====\n",
                (int)ok, (int)_len, (unsigned)program,
                vshader ? (unsigned)vshader->handle : 0u,
                pshader ? (unsigned)pshader->handle : 0u);
            if (_len > 0)
                fprintf(stderr, "%.*s\n===== END LINK LOG =====\n",
                        (int)_len, error_buffer);
        }
#endif
        if (!ok)
        {
...
#ifdef WASM_DIAG_GL
    fprintf(stderr,
        "[MS-compileShader] bufsize=%u smapcount=%u swizcount=%u\n  first128=",
        bufsize, smapcount, swizcount);
    {
        unsigned int _i;
        for (_i = 0; _i < 128 && _i < bufsize; _i++) {
            fprintf(stderr, "%02x ", tokenbuf[_i]);
            if ((_i & 31) == 31) fprintf(stderr, "\n  ");
        }
        fprintf(stderr, "\n");
    }
#endif
...
#ifdef WASM_DIAG_GL
    fprintf(stderr, "===== MOJOSHADER lookup_attributes count=%d =====\n",
            pd->attribute_count);
#endif
...
#ifdef WASM_DIAG_GL
        fprintf(stderr, "  attr[%d] name=%s usage=%d index=%d -> loc=%d\n",
                i, a[i].name ? a[i].name : "(null)",
                (int)a[i].usage, a[i].index, (int)loc);
#endif
